/**
 * Clinovix AI Web Chat — admin settings (Vanilla JS).
...
(function () {
	...
	var select = document.getElementById('clx-provider');
	if (!select) {
		return;
	}

	var blocks = document.querySelectorAll('.clx-provider-block');
...
(function () {
	'use strict';
	if (typeof window.CLX_ADMIN === 'undefined') { return; }
	var A = window.CLX_ADMIN;
	...
	document.querySelectorAll('.clx-act').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var op = btn.getAttribute('data-op');
			var lead = btn.getAttribute('data-lead');
			var original = btn.textContent;
			btn.disabled = true; btn.textContent = A.strings.working;
			var action = op === 'webhook' ? 'clx_export_webhook' : 'clx_export_gsheet';
	...
	var checkAll = document.getElementById('clx-check-all');
	if (checkAll) {
		checkAll.addEventListener('change', function () {
			document.querySelectorAll('.clx-check').forEach(function (c) { c.checked = checkAll.checked; });
		});
	}
	var bulkApply = document.getElementById('clx-bulk-apply');
	if (bulkApply) {
		bulkApply.addEventListener('click', function () {
			var op = document.getElementById('clx-bulk-op').value;
			var ids = [];
			document.querySelectorAll('.clx-check:checked').forEach(function (c) { ids.push(c.value); });
			var out = document.getElementById('clx-bulk-result');
			if (!op || ids.length === 0) { out.textContent = A.strings.noSel; return; }
			bulkApply.disabled = true; out.textContent = A.strings.working;
			post('clx_export_bulk', { op: op, ids: ids }).then(function (res) {
				if (res && res.ok) {
	...
	var crm = document.querySelector('.clx-crm-panel');
	if (crm) {
		var saveBtn = crm.querySelector('.clx-crm-save');
		var result = crm.querySelector('.clx-crm-result');
		saveBtn.addEventListener('click', function () {
			saveBtn.disabled = true; result.textContent = A.strings.working;
			var branchSel = crm.querySelector('.clx-crm-branch');
			var data = {
				lead_id: crm.getAttribute('data-lead'),
				nonce: crm.getAttribute('data-nonce'),
				lead_status: crm.querySelector('.clx-crm-status').value,
				email: crm.querySelector('.clx-crm-email').value,
				tags: crm.querySelector('.clx-crm-tags').value,
				notes: crm.querySelector('.clx-crm-notes').value
			};
			if (branchSel) { data.branch_id = branchSel.value; }
			post('clx_lead_update', data).then(function (res) {
	...
	var smsProvider = document.querySelector('.clx-sms-provider');
	if (smsProvider) {
		var customBox = document.querySelector('.clx-sms-custom');
		var secretRow = document.querySelector('.clx-sms-secret-row');
		var meliHint = document.querySelector('.clx-sms-hint[data-for="melipayamak"]');
	...
	var smsDiagBtn = document.querySelector('.clx-sms-diag-btn');
	if (smsDiagBtn) {
		smsDiagBtn.addEventListener('click', function () {
			var out = document.querySelector('.clx-sms-diag-out');
			smsDiagBtn.disabled = true;
			if (out) { out.style.display = ''; out.textContent = A.strings.working; }
			post('clx_sms_diag', {}).then(function (res) {
				if (out) {
	...
	var smsTestBtn = document.querySelector('.clx-sms-test-btn');
	if (smsTestBtn) {
		smsTestBtn.addEventListener('click', function () {
			var toEl = document.querySelector('.clx-sms-test-to');
			var out = document.querySelector('.clx-test-result[data-for="sms"]');
			var to = toEl ? toEl.value : '';
			if (!to) { if (out) { out.textContent = A.strings.noSel; } return; }
			smsTestBtn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post('clx_test_sms', { to: to }).then(function (res) {
				if (out) {
	...
	document.querySelectorAll('.clx-msgr-test-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var ch = btn.getAttribute('data-channel');
			var out = document.querySelector('.clx-test-result[data-for="msgr-' + ch + '"]');
			btn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post('clx_test_messenger', { channel: ch }).then(function (res) {
				if (out) {
					out.textContent = (res && res.ok) ? ('✓ ' + A.strings.ok) : ('✕ ' + ((res && res.error) || A.strings.failed));
					out.style.color = (res && res.ok) ? '#1a7f37' : '#d63638';
				}
	...
	var refer = document.querySelector('.clx-refer');
	if (refer) {
		var refText = refer.getAttribute('data-text') || '';
		var refResult = refer.querySelector('.clx-refer-result');
		var origResult = refResult ? refResult.textContent : '';

		var mailBtn = refer.querySelector('.clx-refer-mail');
		if (mailBtn) {
			mailBtn.addEventListener('click', function () {
				var to = (refer.querySelector('.clx-refer-email') || {}).value || '';
				if (!to) { refResult.textContent = A.strings.noSel; return; }
				mailBtn.disabled = true; refResult.textContent = A.strings.working;
				post('clx_lead_refer', { lead_id: refer.getAttribute('data-lead'), to: to }).then(function (res) {
		...
		var staffSel = refer.querySelector('.clx-refer-staff');
		var phoneField = refer.querySelector('.clx-refer-phone');
		var emailField = refer.querySelector('.clx-refer-email');
		if (staffSel) {
		...
		var panelBtn = refer.querySelector('.clx-refer-panel');
		if (panelBtn) {
			panelBtn.addEventListener('click', function () {
				var phone = (refer.querySelector('.clx-refer-phone') || {}).value || '';
				if (!phone) { refResult.textContent = A.strings.noSel; return; }
				var tpl = (refer.querySelector('.clx-refer-template') || {}).value || 'referral';
				panelBtn.disabled = true; refResult.textContent = A.strings.working;
				post('clx_send_sms', { lead_id: refer.getAttribute('data-lead'), to: phone, template: tpl }).then(function (res) {
					refResult.textContent = (res && res.ok) ? ('✓ ' + A.strings.ok) : ('✕ ' + ((res && res.error) || A.strings.failed));
					refResult.style.color = (res && res.ok) ? '#1a7f37' : '#d63638';
	...
	var seoGen = document.getElementById('clx-seo-gen');
	if (seoGen) {
		seoGen.addEventListener('click', function () {
			var wrap = document.querySelector('.clx-seo-ai');
			var out = document.getElementById('clx-seo-ideas');
			seoGen.disabled = true;
			var original = seoGen.textContent;
			seoGen.textContent = A.strings.working;
			out.textContent = '…';
			post('clx_seo_generate', { nonce: wrap.getAttribute('data-nonce') }).then(function (res) {
	...
	document.querySelectorAll('.clx-test-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var target = btn.getAttribute('data-target');
			var out = document.querySelector('.clx-test-result[data-for="' + target + '"]');
			btn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post(target === 'webhook' ? 'clx_test_webhook' : 'clx_test_gsheet', {}).then(function (res) {
				if (out) {
