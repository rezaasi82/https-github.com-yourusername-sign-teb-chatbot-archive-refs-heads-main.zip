/**
 * Clinovix — frontend widget (Vanilla JS, no jQuery).
 *
...
(function () {
	...
	if (typeof window.CLX_CONFIG === 'undefined') {
		return;
	}

	var cfg = window.CLX_CONFIG;

	// One page can host several widgets (the floating launcher plus any number
	// of [clinovix_chat] shortcodes), so initialise every instance independently.
	var roots = document.querySelectorAll('.clx-root');
	...
	var panel = root.querySelector('.clx-panel');
	var launcher = root.querySelector('.clx-launcher');
	var closeBtn = root.querySelector('.clx-close');
	var form = root.querySelector('.clx-form');
	var input = root.querySelector('.clx-input');
	var messages = root.querySelector('.clx-messages');
	var quickWrap = root.querySelector('.clx-quick');
	var lead = root.querySelector('.clx-lead');
	...
	// Opt-in debug logging: enable with localStorage.setItem('clx_debug','1')
	// or by setting CLX_CONFIG.debug = true. Traces the full flow so the source
	// of any issue is visible in the console.
	var DEBUG = !!cfg.debug || (function () {
		try { return window.localStorage.getItem('clx_debug') === '1'; } catch (e) { return false; }
	})();
	function log() {
		if (!DEBUG || !window.console) { return; }
		try { console.log.apply(console, ['[Clinovix]'].concat([].slice.call(arguments))); } catch (e) {}
	}
	...
	function getSession() {
		var existing = load('clx_session');
		if (existing) { return existing; }
		var id = 'sess_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
		store('clx_session', id);
		return id;
	}

	function getProfile() {
		try { return JSON.parse(load('clx_profile') || '{}') || {}; } catch (e) { return {}; }
	}
	...
	function lockBody() {
		if (!isMobile() || document.body.classList.contains('clx-body-lock')) { return; }
		savedScrollY = window.scrollY || window.pageYOffset || 0;
		document.body.style.top = '-' + savedScrollY + 'px';
		document.body.classList.add('clx-body-lock');
	}
	function unlockBody() {
		if (!document.body.classList.contains('clx-body-lock')) { return; }
		document.body.classList.remove('clx-body-lock');
	...
	function closePanel() {
		root.classList.remove('clx-open');
		panel.hidden = true;
		panel.style.height = '';
		panel.style.transform = '';
		unlockBody();
	...
	function maybeShowLead() {
		...
		if (lead) {
			lead.hidden = false;
			root.classList.add('clx-lead-active');
			var nameField = lead.querySelector('.clx-lead-name');
	...
	if (lead) {
		var startBtn = lead.querySelector('.clx-lead-start');
		var skipBtn = lead.querySelector('.clx-lead-skip');
		var errBox = lead.querySelector('.clx-lead-error');

		startBtn.addEventListener('click', function () {
			var name = (lead.querySelector('.clx-lead-name').value || '').trim();
			var phone = (lead.querySelector('.clx-lead-phone').value || '').trim();
	...
	function appendMessage(text, who) {
		...
		el.className = 'clx-msg ' + (who === 'user' ? 'clx-msg-user' : 'clx-msg-bot');
	...
	function showTyping() {
		var el = document.createElement('div');
		el.className = 'clx-msg clx-msg-bot clx-typing';
	...
	function renderCtaCard(card) {
		...
		wrap.className = 'clx-cta-card';

		if (root.dataset.chBooking === '1' && bookingUrl) {
			var head = document.createElement('div');
			head.className = 'clx-cta-head';
			head.innerHTML = '<div class="clx-cta-title"></div><div class="clx-cta-text"></div>';
			head.querySelector('.clx-cta-title').textContent = cfg.strings.ctaTitle;
			head.querySelector('.clx-cta-text').textContent = cfg.strings.ctaText;
			wrap.appendChild(head);
			wrap.appendChild(channelBtn(cfg.strings.book, bookingUrl, 'booking', '📅', true));
		}

		var row = document.createElement('div');
		row.className = 'clx-channel-row';
	...
	function channelBtn(label, href, type, emoji, primary) {
		...
		a.className = 'clx-cta-btn clx-cta-' + type + (primary ? ' clx-cta-primary' : '');
		a.href = href;
		a.target = '_blank';
		a.rel = 'noopener';
		a.innerHTML = '<span class="clx-cta-emoji" aria-hidden="true"></span><span class="clx-cta-label"></span>';
		a.querySelector('.clx-cta-emoji').textContent = emoji;
		a.querySelector('.clx-cta-label').textContent = label;
	...
	function ajaxFallback(text) {
		...
		var p = payload(text);
		body.append('action', 'clx_chat_message');
	...
	function trackEvent(type) {
		...
		fetch(cfg.eventUrl, {
			...
			var body = new URLSearchParams();
			body.append('action', 'clx_track_event');
			body.append('nonce', cfg.ajaxNonce);
	...
	if (quickWrap) {
		quickWrap.addEventListener('click', function (e) {
			var btn = e.target.closest('.clx-quick-reply');
	...
	(function initTeaser() {
		...
		function reveal() {
			...
			teaser.hidden = false;
			root.classList.add('clx-teaser-on');
		...
		function dismiss(persist) {
			clearTimeout(timer);
			teaser.hidden = true;
			root.classList.remove('clx-teaser-on');
			if (persist) { store('clx_teaser_dismissed', '1'); }
		}

		var closeBubble = teaser.querySelector('.clx-teaser-close');
		if (closeBubble) {
