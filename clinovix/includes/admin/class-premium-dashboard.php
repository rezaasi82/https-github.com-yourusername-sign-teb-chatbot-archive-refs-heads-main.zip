...
{
    private const PAGE  = 'clx-dashboard';
    private const NONCE = 'clx_dashboard';

    public function register(): void
    {
        add_action('admin_menu', [$this, 'menu'], 20);
        add_action('admin_enqueue_scripts', [$this, 'enqueue']);
        add_action('wp_ajax_clx_integrity_check', [$this, 'ajax_integrity']);
    }
    ...
    public function menu(): void
    {
        add_submenu_page(
            'clx-chat',
            __('داشبورد Clinovix AI', 'clinovix'),
            __('داشبورد', 'clinovix'),
            'manage_options',
            self::PAGE,
            [$this, 'render'],
    ...
    public function enqueue(string $hook): void
    {
        ...
        wp_enqueue_style('clx-dashboard', CLX_URL . 'assets/css/dashboard.css', [], CLX_VERSION);
        wp_enqueue_script('clx-dashboard', CLX_URL . 'assets/js/dashboard.js', [], CLX_VERSION, true);
        wp_localize_script('clx-dashboard', 'CLX_DASH', [
    ...
    public function render(): void
    {
        ...
        include CLX_DIR . 'includes/admin/views/premium-dashboard.php';
    }
    ...
    {
        $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('نامعتبر', 'clinovix'), 'code' => 0];

        try {
            $env_ok = defined('CLX_VERSION')
                && defined('CLX_FILE')
                && class_exists('\Clinovix\Core\Plugin');

            if ($env_ok === true) {
                $verdict = ['ok' => true, 'level' => 'secure', 'label' => __('فعال و ایمن', 'clinovix'), 'code' => 200];
            }
        } catch (\Throwable $e) {
            $this->log_anomaly('integrity_gate', $e);
            $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('خطای بررسی', 'clinovix'), 'code' => 500];
        }
    ...
    private function metrics(int $days): array
    {
        ...
        $data = \Clinovix\Core\Cache::remember('dash_metrics_' . $days, 300, function () use ($days) {
            $m = [
            ...
            try {
                ...
                $avg_price    = (int) (new \Clinovix\Core\Settings())->get('avg_service_price', 0);
    ...
    private function log_anomaly(string $context, \Throwable $e): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log(sprintf('[Clinovix] %s anomaly: %s', $context, $e->getMessage()));
        }
    }
}
