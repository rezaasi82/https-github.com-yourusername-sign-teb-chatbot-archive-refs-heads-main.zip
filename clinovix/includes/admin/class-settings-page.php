...
class SettingsPage
{
    ...
    {
        $tab = \Clinovix\Core\Input::get_key('tab', 'provider');
        return in_array($tab, self::TABS, true) ? $tab : 'provider';
    }

    public function handle_save(): void
    {
        if (! \Clinovix\Core\Input::has_post('clx_settings_submit')) {
            return;
        }
        if (! current_user_can('manage_options')) {
            return;
        }
        check_admin_referer('clx_settings');
        ...
        $existing = get_option(\Clinovix\Core\Settings::OPTION, []);
        ...
        if ($tab === 'provider') {
            ...
            if (isset($in['api_key_anthropic']) && trim((string) $in['api_key_anthropic']) !== '') {
                \Clinovix\Core\Settings::save_api_key('anthropic', sanitize_text_field((string) $in['api_key_anthropic']));
            }
            if (isset($in['api_key_openai']) && trim((string) $in['api_key_openai']) !== '') {
                \Clinovix\Core\Settings::save_api_key('openai', sanitize_text_field((string) $in['api_key_openai']));
            }
            if (isset($in['api_key_gapgpt']) && trim((string) $in['api_key_gapgpt']) !== '') {
                \Clinovix\Core\Settings::save_api_key('gapgpt', sanitize_text_field((string) $in['api_key_gapgpt']));
            }
        } elseif ($tab === 'clinic') {
        ...
        update_option(\Clinovix\Core\Settings::OPTION, array_merge($existing, $update));
        $this->finish($tab);
    }
    ...
    private function save_integrations(array $in): void
    {
        ...
        if (isset($in['sms_key']) && trim((string) $in['sms_key']) !== '') {
            \Clinovix\Notifications\SmsManager::save_key((string) $in['sms_key']);
        }
        if (isset($in['sms_secret']) && trim((string) $in['sms_secret']) !== '') {
            \Clinovix\Notifications\SmsManager::save_secret((string) $in['sms_secret']);
        }
        ...
        foreach (['bale', 'telegram'] as $ch) {
            if (isset($in['msgr_' . $ch . '_token']) && trim((string) $in['msgr_' . $ch . '_token']) !== '') {
                \Clinovix\Notifications\MessengerNotifier::save_token($ch, (string) $in['msgr_' . $ch . '_token']);
            }
        }

        if (isset($in['webhook_secret']) && trim((string) $in['webhook_secret']) !== '') {
            \Clinovix\Export\WebhookManager::save_secret((string) $in['webhook_secret']);
        }
        if (isset($in['gsheet_secret']) && trim((string) $in['gsheet_secret']) !== '') {
            \Clinovix\Export\GoogleSheets::save_secret((string) $in['gsheet_secret']);
        }
        if (isset($in['cloud_secret']) && trim((string) $in['cloud_secret']) !== '') {
            \Clinovix\Cloud\CloudClient::save_secret((string) $in['cloud_secret']);
        }
    }

    private function finish(string $tab): void
    {
        \Clinovix\Security\AuditLog::record('settings_saved', ['object' => $tab, 'severity' => 'info']);
        add_settings_error('pzr', 'saved', __('تنظیمات ذخیره شد.', 'clinovix'), 'updated');
        set_transient('settings_errors', get_settings_errors(), 30);
        wp_safe_redirect(admin_url('admin.php?page=clx-chat&tab=' . $tab . '&updated=1'));
        exit;
    }
    ...
    public function render(): void
    {
        ...
        $s       = new \Clinovix\Core\Settings();

        echo '<div class="wrap clx-admin" dir="rtl">';
        ...
        if ($float && $shortcode) {
            $display = __('ویجت شناور + شورت‌کد', 'clinovix');
        } elseif ($float) {
            $display = __('فقط ویجت شناور', 'clinovix');
        } elseif ($shortcode) {
            $display = __('فقط شورت‌کد', 'clinovix');
        } else {
            $display = __('نمایش خاموش', 'clinovix');
        }

        \Clinovix\Admin\PageHeader::render(
            __('Clinovix AI', 'clinovix'),
            __('دستیار هوشمند جذب بیمار', 'clinovix'),
            [
                [
                    'label' => $display,
                ...
                [
                    'label' => $has_key
                        ? __('کلید API تنظیم شده', 'clinovix')
                        : __('کلید API تنظیم نشده', 'clinovix'),
                    'state' => $has_key ? 'on' : 'off',
                ],
                ['label' => __('نسخه', 'clinovix') . ' ' . CLX_VERSION],
            ]
        );

        // Pistachio-green success toast after a save (auto-dismisses via CSS).
        if (\Clinovix\Core\Input::get_key('updated') === '1') {
            echo '<div class="clx-saved-toast" role="status">'
                . '<span class="clx-saved-ico" aria-hidden="true">✓</span>'
                . '<span>' . esc_html__('تغییرات با موفقیت ذخیره شد.', 'clinovix') . '</span>'
                . '</div>';
        } else {
            settings_errors('pzr');
        }
        ...
        if (in_array($tab, ['provider', 'clinic', 'appearance', 'integrations'], true)) {
            include CLX_DIR . 'includes/admin/views/settings.php';
        } elseif ($tab === 'conversations') {
            (new \Clinovix\Admin\ConversationsPage())->render_inner();
        } elseif ($tab === 'stats') {
            (new \Clinovix\Admin\StatsPage())->render_inner();
        }
    ...
    {
        $labels = [
            'provider'      => __('هوش مصنوعی', 'clinovix'),
            'clinic'        => __('اطلاعات کلینیک', 'clinovix'),
            'appearance'    => __('ظاهر ویجت', 'clinovix'),
            'integrations'  => __('اتصال‌ها و خروجی', 'clinovix'),
            'conversations' => __('لیدها و مکالمات', 'clinovix'),
            'stats'         => __('آمار', 'clinovix'),
        ];
        echo '<h2 class="nav-tab-wrapper">';
        foreach ($labels as $slug => $label) {
            $url    = admin_url('admin.php?page=clx-chat&tab=' . $slug);
