...
class AiManager
{
    ...
    private \Clinovix\Safety\MedicalSafetyFilter $safety;
    private \Clinovix\Ai\SystemPromptBuilder $prompt;
    private \Clinovix\Ai\LanguageDetector $language;
    private \Clinovix\Ai\CtaDetector $cta;
    private \Clinovix\Ai\LeadScorer $scorer;
    private \Clinovix\Ai\SummaryBuilder $summary;
    private \Clinovix\Ai\ProviderFactory $providers;
    private \Clinovix\Database\ConversationRepository $conversations;
    private \Clinovix\Database\MessageRepository $messages;

    public function __construct()
    {
        $this->settings      = new \Clinovix\Core\Settings();
        $this->safety        = new \Clinovix\Safety\MedicalSafetyFilter($this->settings);
        $this->prompt        = new \Clinovix\Ai\SystemPromptBuilder($this->settings);
        $this->language      = new \Clinovix\Ai\LanguageDetector();
        $this->cta           = new \Clinovix\Ai\CtaDetector();
        $this->scorer        = new \Clinovix\Ai\LeadScorer();
        $this->summary       = new \Clinovix\Ai\SummaryBuilder($this->settings);
        $this->providers     = new \Clinovix\Ai\ProviderFactory($this->settings);
        $this->conversations = new \Clinovix\Database\ConversationRepository();
        $this->messages      = new \Clinovix\Database\MessageRepository();
    }
    ...
    public function handle(array $req): array
    {
        if (! $this->settings->is_enabled()) {
            return ['ok' => false, 'code' => 'disabled', 'error' => __('چت‌بات غیرفعال است.', 'clinovix')];
        }

        $message = trim((string) $req['message']);
        if ($message === '') {
            return ['ok' => false, 'code' => 'empty', 'error' => __('پیام خالی است.', 'clinovix')];
        }
        ...
        $limiter = new \Clinovix\Ratelimit\RateLimiter((int) $this->settings->get('rate_limit_per_min', 8));
        if (! $limiter->allow($req['ip'] . '|' . $req['session_id'])) {
            return ['ok' => false, 'code' => 'rate_limited', 'error' => __('لطفاً کمی صبر کنید و دوباره تلاش کنید.', 'clinovix')];
        }
        ...
        $context = [
            ...
            'max_tokens'  => 1200,
            'temperature' => (float) apply_filters('clx_temperature', 0.8),
        ];
        ...
        if ($cta !== '') {
            ...
            do_action('clx_lead_detected', $conversation_id, $cta);
        }
        ...
        do_action('clx_message_handled', $conversation_id, $cta, $score['level']);
