<?php
/**
 * The plugin side of the Clinovix Cloud platform (Level 2).
 *
...
class CloudClient
{
    public const OPTION_SECRET   = 'clx_cloud_secret_enc';
    public const OPTION_REGISTERED = 'clx_cloud_registered';
    private const CRON_HEARTBEAT = 'clx_cloud_heartbeat';
    private const CRON_INSTALL   = 'clx_cloud_install';
    ...
    public static function save_secret(string $plain): void
    {
        ...
        update_option(self::OPTION_SECRET, \Clinovix\Core\Encryption::encrypt($plain), false);
    }
    ...
    private function telemetry(): array
    {
        return [
            ...
            'plugin_version' => defined('CLX_VERSION') ? CLX_VERSION : '',
    ...
    private function counts(): array
    {
        ...
        $msg  = \Clinovix\Database\Schema::messages_table();
        return [
            ...
            'active_24h' => (new \Clinovix\Database\ConversationRepository())->active_count(24),
        ];
    }
    ...
    private function send(string $type, array $payload): bool
    {
        ...
        $headers = [
            ...
            'X-Clinovix-Type'  => $type,
            'X-Clinovix-Time'  => (string) time(),
        ];
        if ($secret !== '') {
            $headers['X-Clinovix-Sign'] = 'sha256=' . hash_hmac('sha256', $body, $secret);
        }
    ...
    private function log(\Throwable $e): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Clinovix] cloud client: ' . $e->getMessage());
        }
    }
}
