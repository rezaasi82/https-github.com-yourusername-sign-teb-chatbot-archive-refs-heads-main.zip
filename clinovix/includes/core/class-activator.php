...
class Activator
{
    ...
    {
        \Clinovix\Database\Schema::install();
        self::seed_default_settings();
        if (! wp_next_scheduled(\Clinovix\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Clinovix\Jobs\Rollup::CRON);
        }
        flush_rewrite_rules();
    }
    ...
    {
        if (get_option('clx_db_version') !== \Clinovix\Database\Schema::DB_VERSION) {
            \Clinovix\Database\Schema::install();
        }
        if (! wp_next_scheduled(\Clinovix\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Clinovix\Jobs\Rollup::CRON);
    ...
    public static function default_settings(): array
    {
        return [
            ...
            'bot_name'           => __('دستیار هوشمند', 'clinovix'),
            'avatar_url'         => '',
            ...
            // --- Messaging ---
            'welcome_message'    => __('سلام! 👋 چطور می‌تونم کمکتون کنم؟', 'clinovix'),
            'quick_replies'      => "هزینه ویزیت\nآدرس کلینیک\nرزرو نوبت",
            'business_hours'     => '',
            'offhours_message'   => __('در حال حاضر خارج از ساعت کاری هستیم، اما می‌تونم به سوالاتتون پاسخ بدم.', 'clinovix'),
    ...
    private static function seed_default_settings(): void
    {
        ...
        update_option(\Clinovix\Core\Settings::OPTION, array_merge(self::default_settings(), $existing));
    }
}
