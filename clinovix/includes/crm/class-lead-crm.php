...
class LeadCrm
{
    ...
    {
        add_action('wp_ajax_clx_lead_update', [$this, 'ajax_update']);
        add_action('wp_ajax_clx_lead_refer', [$this, 'ajax_refer']);
    }
    ...
    public static function referral_text(object $c): string
    {
        $scores = [
            'hot'  => __('داغ', 'clinovix'),
            'warm' => __('متوسط', 'clinovix'),
            'cold' => __('سرد', 'clinovix'),
        ];
        $lines = [
            __('ارجاع لید از Clinovix', 'clinovix'),
            sprintf(__('نام: %s', 'clinovix'), trim((string) ($c->patient_name ?? '')) ?: '—'),
            sprintf(__('موبایل: %s', 'clinovix'), trim((string) ($c->patient_phone ?? '')) ?: '—'),
            sprintf(__('امتیاز: %s', 'clinovix'), $scores[$c->lead_score ?? ''] ?? '—'),
            sprintf(__('وضعیت: %s', 'clinovix'), self::label((string) ($c->lead_status ?? 'new'))),
        ];
        $summary = trim((string) ($c->summary ?? ''));
        if ($summary !== '') {
            $lines[] = "\n" . __('خلاصه گفتگو:', 'clinovix') . "\n" . $summary;
        }
        return implode("\n", $lines);
    }
    ...
    public function ajax_refer(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Clinovix\Security\Security::note_failure('crm_refer');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }

        $lead_id = \Clinovix\Core\Input::post_int('lead_id');
        $to      = \Clinovix\Core\Input::post_email('to');
        if ($lead_id <= 0) {
            wp_send_json(['ok' => false, 'error' => 'bad_lead'], 400);
        }
        if (! is_email($to)) {
            wp_send_json(['ok' => false, 'error' => __('ایمیل مقصد نامعتبر است.', 'clinovix')], 400);
        }
        ...
        $subject = sprintf(__('[%s] ارجاع لید — %s', 'clinovix'), $clinic, trim((string) ($c->patient_name ?? '')) ?: ('#' . $lead_id));
        $body    = self::referral_text($c) . "\n\n" . admin_url('admin.php?page=clx-chat&tab=conversations&conversation=' . $lead_id);

        $sent = wp_mail($to, $subject, $body);
        \Clinovix\Security\AuditLog::record('lead_refer', ['object' => (string) $lead_id, 'severity' => $sent ? 'info' : 'warning']);

        if (! $sent) {
            wp_send_json(['ok' => false, 'error' => __('ارسال ایمیل ناموفق بود. تنظیمات ایمیل سایت را بررسی کنید.', 'clinovix')], 500);
        }
    ...
    public function ajax_update(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Clinovix\Security\Security::note_failure('crm_update');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        ...
        if (\Clinovix\Core\Input::has_post('lead_status')) {
            $status = \Clinovix\Core\Input::post_key('lead_status');
            if (self::is_valid($status)) {
                $fields['lead_status'] = $status;
            }
        }
        if (\Clinovix\Core\Input::has_post('email')) {
            $fields['email'] = \Clinovix\Core\Input::post_email('email');
        }
        if (\Clinovix\Core\Input::has_post('tags')) {
            $fields['tags'] = \Clinovix\Core\Input::post_text('tags');
        }
        if (\Clinovix\Core\Input::has_post('notes')) {
            $fields['notes'] = \Clinovix\Core\Input::post_textarea('notes');
        }
        if (\Clinovix\Core\Input::has_post('branch_id')) {
            $branch = \Clinovix\Core\Input::post_int('branch_id');
            if ($branch === 0 || (new \Clinovix\Database\BranchRepository())->exists($branch)) {
        ...
        do_action('clx_lead_crm_updated', $lead_id, $fields);
