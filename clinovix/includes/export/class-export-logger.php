...
class ExportLogger
{
    private \Clinovix\Database\SyncLogRepository $repo;

    public function __construct(?\Clinovix\Database\SyncLogRepository $repo = null)
    {
        $this->repo = $repo ?? new \Clinovix\Database\SyncLogRepository();
    }
    ...
    public function finish(int $log_id, string $status, array $extra = []): void
    {
        ...
        if ($status === 'failed' && defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Clinovix export] log #' . $log_id . ' failed: ' . ($update['response'] ?? ''));
        }
    }
}
