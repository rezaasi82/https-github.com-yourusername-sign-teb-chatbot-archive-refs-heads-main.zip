...
class ExportManager
{
    ...
    {
        add_action('clx_webhook_retry', [$this, 'run_webhook_retry'], 10, 4);

        // Fire when a conversation is first detected as a lead.
        add_action('clx_lead_detected', [$this, 'on_lead_detected'], 20, 2);
    }

    public function run_webhook_retry(int $lead_id, string $event, string $pdf_url = '', int $attempt = 2): void
    {
        (new \Clinovix\Export\WebhookManager($this->settings))->handle_retry($lead_id, $event, $pdf_url, $attempt);
    }
    ...
    {
        $webhook = new \Clinovix\Export\WebhookManager($this->settings);
        ...
        $sheets = new \Clinovix\Export\GoogleSheets($this->settings);
    ...
    {
        $logger  = new \Clinovix\Export\ExportLogger();
        $log_id  = $logger->begin($lead_id, 'pdf', 'manual');
        $started = microtime(true);

        $result = (new \Clinovix\Export\PdfGenerator($this->settings))->generate($lead_id);
        ...
        do_action('clx_pdf_generated', $lead_id, $result['url']);
    ...
    {
        return (new \Clinovix\Export\WebhookManager($this->settings))->dispatch($lead_id, $event, (string) $this->pdf_url($lead_id));
    }
    ...
    {
        return (new \Clinovix\Export\GoogleSheets($this->settings))->dispatch($lead_id, (string) $this->pdf_url($lead_id));
    }
