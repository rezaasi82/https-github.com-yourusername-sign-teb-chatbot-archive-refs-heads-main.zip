...
class GoogleSheets
{
    public const OPTION_SECRET = 'clx_gsheet_secret_enc';
    ...
    public function send(array $row): array
    {
        ...
        $secret  = $this->secret();
        if ($secret !== '') {
            $headers['X-Clinovix-Secret'] = $secret;
        }
    ...
    {
        return $this->send(['TEST', 'Clinovix', '-', 'test', '-', current_time('mysql'), 'connection test', '-', 'test']);
    }
