 ...
 *
 * A lead is a conversation (clx_conversations) plus its messages
 * (clx_messages). This is the single representation consumed by every export
...
class LeadPayload
{
    ...
    public function __construct(?\Clinovix\Database\ConversationRepository $conversations = null, ?\Clinovix\Database\MessageRepository $messages = null)
    {
        $this->conversations = $conversations ?? new \Clinovix\Database\ConversationRepository();
        $this->messages      = $messages ?? new \Clinovix\Database\MessageRepository();
    }
