 ...
 * under uploads/clinovix-pdf/YYYY/MM/ with an unguessable name.
 *
 * @package Clinovix
 */
...
class PdfGenerator
{
    ...
    public function __construct(?\Clinovix\Core\Settings $settings = null, ?\Clinovix\Export\LeadPayload $payloads = null)
    {
    ...
    private function target_dir(): ?array
    {
        ...
        $sub  = '/clinovix-pdf/' . gmdate('Y') . '/' . gmdate('m');
        $path = $uploads['basedir'] . $sub;
        if (! wp_mkdir_p($path)) {
            return null;
        }
        // Prevent directory listing.
        if (! file_exists($uploads['basedir'] . '/clinovix-pdf/index.php')) {
            file_put_contents($uploads['basedir'] . '/clinovix-pdf/index.php', "<?php\n// Silence is golden.\n");
        }
    ...
    public function render_html(array $payload): string
    {
        ...
        foreach ($payload['messages'] as $m) {
            $is_user = $m['role'] === 'user';
            $who     = $is_user ? 'بیمار' : 'دستیار Clinovix';
            $side    = $is_user ? 'clx-p' : 'clx-a';
...
<style>
    * { box-sizing: border-box; }
    ...
    .clx-p { background: #eef2fb; border: 1px solid #dbe2f2; margin-inline-start: auto; }
    .clx-a { background: #fff; border: 1px solid #e6e9f0; }
...
<div class="report">
    <div class="head">
        <div class="brand"><?php echo esc_html($clinic); ?><small>CLINOVIX</small></div>
    ...
    <div class="foot">
        <span>Clinovix · <?php echo esc_html($exported); ?></span>
