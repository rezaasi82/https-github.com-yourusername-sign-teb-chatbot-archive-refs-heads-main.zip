...
{
    public const OPTION_SECRET = 'clx_webhook_secret_enc';
    public const MAX_ATTEMPTS  = 3;

    private \Clinovix\Core\Settings $settings;
    private \Clinovix\Export\ExportLogger $logger;
    private \Clinovix\Export\LeadPayload $payloads;

    public function __construct(?\Clinovix\Core\Settings $settings = null, ?\Clinovix\Export\ExportLogger $logger = null, ?\Clinovix\Export\LeadPayload $payloads = null)
    {
        $this->settings = $settings ?? new \Clinovix\Core\Settings();
        $this->logger   = $logger ?? new \Clinovix\Export\ExportLogger();
        $this->payloads = $payloads ?? new \Clinovix\Export\LeadPayload();
    }
    ...
    public function dispatch(int $lead_id, string $event, string $pdf_url = '', int $attempt = 1): array
    {
        ...
        if ($should_retry) {
            wp_schedule_single_event(time() + 300 * $attempt, 'clx_webhook_retry', [$lead_id, $event, $pdf_url, $attempt + 1]);
        }
    ...
    public function send(array $payload, string $event = 'manual'): array
    {
        ...
        $headers = [
            ...
            'X-Clinovix-Event'  => $event,
        ];
        if ($secret !== '') {
            $headers['X-Clinovix-Signature'] = 'sha256=' . hash_hmac('sha256', (string) $body, $secret);
        }
