 ...
 * drained in small batches on the clx_process_jobs cron, so an admin request
...
class JobQueue
{
    public const CRON  = 'clx_process_jobs';
    ...
    public function enqueue(string $type, array $payload): int
    {
        ...
        $wpdb->insert(
            \Clinovix\Database\Schema::jobs_table(),
            [
    ...
    {
        global $wpdb;
        $table = \Clinovix\Database\Schema::jobs_table();
        ...
        foreach ($jobs as $job) {
            ...
            try {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('[Clinovix] job #' . $job->id . ' error: ' . $e->getMessage());
                }
            }
