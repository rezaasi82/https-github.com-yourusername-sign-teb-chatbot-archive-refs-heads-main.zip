...
namespace Clinovix\Notifications;
...
class SmsManager
{
    public const OPTION_KEY    = 'clx_sms_key_enc';
    public const OPTION_SECRET = 'clx_sms_secret_enc';

    /** id => provider class. Add a panel by adding one line + one class. */
    private const PROVIDERS = [
        'kavenegar'   => '\Clinovix\Notifications\Providers\SmsKavenegar',
        'melipayamak' => '\Clinovix\Notifications\Providers\SmsMelipayamak',
        'smsir'       => '\Clinovix\Notifications\Providers\SmsSmsir',
        'ghasedak'    => '\Clinovix\Notifications\Providers\SmsGhasedak',
        'custom'      => '\Clinovix\Notifications\Providers\SmsCustom',
    ];

    private \Clinovix\Core\Settings $settings;

    public function __construct(?\Clinovix\Core\Settings $settings = null)
    {
        $this->settings = $settings ?? new \Clinovix\Core\Settings();
    }
    ...
    public function active(): ?\Clinovix\Notifications\SmsProviderInterface
    {
    ...
    public function send(string $to, string $text): array
    {
        $text = trim($text);
        if ($text === '') {
            return ['ok' => false, 'error' => __('متن پیام خالی است.', 'clinovix')];
        }
        return $this->dispatch(static fn(\Clinovix\Notifications\SmsProviderInterface $p) => $p->send($to, $text));
    }
    ...
    public function send_lead(string $to, string $template_key, array $vars): array
    {
        ...
        return $this->dispatch(function (\Clinovix\Notifications\SmsProviderInterface $p) use ($to, $tpl_text, $code, $vars, $order) {
    ...
    private function dispatch(callable $call): array
    {
        if (! $this->is_configured()) {
            return ['ok' => false, 'error' => __('پنل پیامک پیکربندی نشده است.', 'clinovix')];
        }
        $provider = $this->active();
        if ($provider === null) {
            return ['ok' => false, 'error' => __('سرویس پیامک نامعتبر است.', 'clinovix')];
        }
        $result = $call($provider);
        \Clinovix\Security\AuditLog::record('sms_send', ['object' => $provider->id(), 'severity' => ! empty($result['ok']) ? 'info' : 'warning']);
        return $result;
    }
    ...
    {
        $lines   = [];
        $lines[] = sprintf(__('سرویس انتخاب‌شده: %s', 'clinovix'), $this->providers()[$this->active_id()] ?? $this->active_id());
        $lines[] = sprintf(__('فعال‌سازی: %s', 'clinovix'), $this->is_enabled() ? '✓' : __('✗ (تیک «فعال‌سازی» را بزنید)', 'clinovix'));
        $lines[] = sprintf(__('APIKey ذخیره شده: %s', 'clinovix'), self::key() !== '' ? '✓' : '✗');
        $lines[] = sprintf(__('رمز عبور ذخیره شده: %s', 'clinovix'), self::secret() !== '' ? __('✓ (حالت نام‌کاربری/رمز)', 'clinovix') : __('— (حالت توکن)', 'clinovix'));
        $lines[] = sprintf(__('شماره فرستنده: %s', 'clinovix'), trim((string) $this->settings->get('sms_sender', '')) !== '' ? $this->settings->get('sms_sender') : __('خالی (برای خط اشتراکی درست است)', 'clinovix'));

        $codes = array_filter(array_map([$this, 'template_code'], array_keys(self::default_templates())));
        $lines[] = sprintf(__('کد الگو تنظیم‌شده: %d قالب', 'clinovix'), count($codes));
        ...
        $lines[] = __('این سرویس بررسی زنده ندارد؛ با «ارسال پیامک تست» امتحان کنید.', 'clinovix');
    ...
    public static function key(): string
    {
        ...
        return is_string($enc) && $enc !== '' ? \Clinovix\Core\Encryption::decrypt($enc) : '';
    }
    ...
    public static function default_templates(): array
    {
        ...
        return [
            'welcome'  => [
                'label' => __('خوش‌آمد به لید', 'clinovix'),
                'text'  => __('{0} عزیز، از تماس شما با {1} سپاسگزاریم. کارشناسان ما به‌زودی برای هماهنگی با شما تماس می‌گیرند. 🌿 {optout}', 'clinovix'),
                'vars'  => ['name', 'clinic'],
            ],
            'referral' => [
                'label' => __('ارجاع لید به همکار', 'clinovix'),
                'text'  => __('لید جدید در سایت : نام {0} -شماره {1} - امتیاز: {2}. لطفاً پیگیری کنید. {optout}', 'clinovix'),
                'vars'  => ['name', 'phone', 'score'],
            ],
            'reminder' => [
                'label' => __('یادآوری پیگیری', 'clinovix'),
                'text'  => __('{0} عزیز، جهت تکمیل مشاوره و رزرو نوبت با {1} در ارتباط باشید. منتظر شما هستیم. {optout}', 'clinovix'),
                'vars'  => ['name', 'clinic'],
            ],
            'custom'   => [
                'label' => __('پیام سفارشی', 'clinovix'),
                'text'  => __('{0} عزیز، {1} در خدمت شماست. {optout}', 'clinovix'),
                'vars'  => ['name', 'clinic'],
            ],
        ];
    }
    ...
    {
        $scores = ['hot' => __('داغ', 'clinovix'), 'warm' => __('متوسط', 'clinovix'), 'cold' => __('سرد', 'clinovix')];
        return [
            'name'    => trim((string) ($c->patient_name ?? '')) ?: __('کاربر', 'clinovix'),
            'phone'   => trim((string) ($c->patient_phone ?? '')),
            'score'   => $scores[$c->lead_score ?? ''] ?? '—',
            'status'  => \Clinovix\Crm\LeadCrm::label((string) ($c->lead_status ?? 'new')),
    ...
    public static function messenger_links(string $phone, string $text): array
    {
        ...
        return [
            'sms'      => ['label' => __('پیامک دستگاه', 'clinovix'), 'url' => 'sms:' . $digits . '?&body=' . $enc],
