...
class SmsCustom extends \Clinovix\Notifications\SmsProviderBase
{
    ...
    {
        return __('سفارشی / خارجی (Custom HTTP)', 'clinovix');
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($url === '') {
            return ['ok' => false, 'error' => __('آدرس سرویس سفارشی تنظیم نشده است.', 'clinovix')];
        }
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $r['error'] ?? sprintf(__('پاسخ سرویس: HTTP %d', 'clinovix'), $r['code'])];
    }
