...
namespace Clinovix\Notifications\Providers;

if (! defined('ABSPATH')) {
    exit;
}

class SmsMelipayamak extends \Clinovix\Notifications\SmsProviderBase
{
    ...
    public function check(): array
    {
        ...
        if ($key === '') {
            return ['ok' => false, 'detail' => __('هیچ APIKey/نام کاربری ذخیره نشده است.', 'clinovix')];
        }
        ...
        if (! $this->is_legacy() && $this->looks_like_webservice_key($key)) {
            return ['ok' => false, 'detail' => __('این کد «APIKey وب‌سرویس» ملی‌پیامک است، نه توکن کنسول. نام کاربری پنل (معمولاً شماره موبایل) را در فیلد APIKey و همین کد را در فیلد «رمز عبور» وارد کنید و ذخیره کنید.', 'clinovix')];
        }

        if ($this->is_legacy()) {
            $probe = $this->get_credit($key, $secret);
            if ($probe['neterr'] !== '') {
                return ['ok' => false, 'detail' => sprintf(__('سرور سایت به پنل دسترسی ندارد: %s', 'clinovix'), $probe['neterr'])];
            }
            if ($probe['ok']) {
                return ['ok' => true, 'detail' => sprintf(__('حالت نام‌کاربری/رمز ✓ — اعتبار پنل: %s', 'clinovix'), $probe['credit'])];
            }
            ...
            if ($this->looks_like_webservice_key($key) && ! $this->looks_like_webservice_key($secret)) {
                ...
                if ($swapped['ok']) {
                    \Clinovix\Notifications\SmsManager::save_key($secret);
                    \Clinovix\Notifications\SmsManager::save_secret($key);
                    return ['ok' => true, 'detail' => sprintf(__('جای دو فیلد برعکس بود؛ به‌طور خودکار اصلاح و ذخیره شد ✓ — اعتبار پنل: %s', 'clinovix'), $swapped['credit'])];
                }
            }

            return ['ok' => false, 'detail' => sprintf(__('پنل اتصال را رد کرد (GetCredit): %s — چیدمان درست: نام کاربری پنل در فیلد APIKey، و «APIKey وب‌سرویس» (کد UUID از تنظیمات وبسرویس پنل) در فیلد رمز عبور. اگر باز رد شد، وب‌سرویس را در پنل فعال و در صورت نیاز IP سرور سایت را مجاز کنید.', 'clinovix'), $probe['msg'])];
        }

        return ['ok' => true, 'detail' => __('حالت توکن کنسول شناسایی شد (رمز خالی است). برای آزمون واقعی، «ارسال پیامک تست» را بزنید.', 'clinovix')];
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($key === '' || $to === '') {
            return ['ok' => false, 'error' => __('APIKey (یا نام کاربری) و شماره مقصد لازم است.', 'clinovix')];
        }

        if ($this->is_legacy()) {
            if ($this->sender() === '') {
                return ['ok' => false, 'error' => __('برای ارسال متن آزاد، شماره خط لازم است. با خط خدماتی اشتراکی، «کد الگو» را برای قالب تنظیم کنید.', 'clinovix')];
            }
        ...
        if ($this->sender() === '') {
            return ['ok' => false, 'error' => __('برای ارسال متن آزاد، شماره خط اختصاصی لازم است. با خط خدماتی اشتراکی، «کد الگو» را برای قالب تنظیم کنید تا از مسیر الگویی ارسال شود.', 'clinovix')];
        }
    ...
    public function send_pattern(string $to, string $code, array $params): array
    {
        ...
        $to  = $this->normalize($to);
        if ($key === '' || $to === '' || $code === '') {
            return ['ok' => false, 'error' => __('APIKey (یا نام کاربری)، شماره یا کد الگو تنظیم نشده است.', 'clinovix')];
        }
    ...
    private function parse_legacy(array $r): array
    {
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $msg !== '' && $msg !== 'Ok' ? $msg : ($r['error'] ?? __('ارسال ناموفق بود.', 'clinovix'))];
    }
    ...
    private function parse_console(array $r): array
    {
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $msg !== '' ? $msg : ($r['error'] ?? __('ارسال ناموفق بود.', 'clinovix'))];
    }
}
