...
class SmsSmsir extends \Clinovix\Notifications\SmsProviderBase
{
    ...
    public function check(): array
    {
        ...
        if ((int) ($data['status'] ?? 0) === 1) {
            return ['ok' => true, 'detail' => sprintf(__('اتصال برقرار ✓ — اعتبار: %s', 'clinovix'), (string) ($data['data'] ?? '?'))];
        }
        return ['ok' => false, 'detail' => sprintf(__('پنل اتصال را رد کرد: %s', 'clinovix'), (string) ($data['message'] ?? ('HTTP ' . $r['code'])))];
    }
