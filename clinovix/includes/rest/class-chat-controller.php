...
class ChatController
{
    ...
    public function handle_message(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $result = (new \Clinovix\Ai\AiManager())->handle([
            'session_id' => \Clinovix\Rest\Sanitizer::session_id((string) $request->get_param('session_id')),
            'message'    => sanitize_textarea_field((string) $request->get_param('message')),
            'name'       => \Clinovix\Rest\Sanitizer::name((string) $request->get_param('name')),
            'phone'      => \Clinovix\Rest\Sanitizer::phone((string) $request->get_param('phone')),
