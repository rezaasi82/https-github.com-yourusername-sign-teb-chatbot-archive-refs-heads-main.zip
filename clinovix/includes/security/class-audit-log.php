...
class AuditLog
{
    private const PAGE = 'clx-security';
    ...
    public function register(): void
    {
        ...
        add_action('clx_pdf_generated', static function ($lead_id): void {
    ...
    public static function record(string $action, array $args = []): void
    {
        ...
        $wpdb->insert(
            \Clinovix\Database\Schema::audit_logs_table(),
            [
                'user_id'    => get_current_user_id() ?: null,
                'action'     => substr($action, 0, 48),
                'object'     => isset($args['object']) ? substr((string) $args['object'], 0, 64) : null,
                'severity'   => $severity,
                'ip'         => class_exists('\Clinovix\Security\Security') ? \Clinovix\Security\Security::ip() : null,
        ...
        if (wp_rand(1, 50) === 1) {
            $table = \Clinovix\Database\Schema::audit_logs_table();
    ...
    public function render(): void
    {
        ...
        include CLX_DIR . 'includes/admin/views/audit-log.php';
    }
}
