...
class Security
{
    ...
    {
        return class_exists('\Clinovix\Rest\Sanitizer') ? \Clinovix\Rest\Sanitizer::client_ip() : '0.0.0.0';
    }
    ...
    {
        $key   = 'clx_rl_' . md5($bucket . '|' . self::ip());
    ...
    {
        return (bool) get_transient('clx_lock_' . md5(self::ip()));
    }
    ...
    {
        $ip    = self::ip();
        $key   = 'clx_fail_' . md5($ip);
        $count = (int) get_transient($key) + 1;
        set_transient($key, $count, self::WINDOW);

        if ($count >= self::MAX_FAILURES) {
            set_transient('clx_lock_' . md5($ip), 1, self::LOCK);
            delete_transient($key);
            if (class_exists('\Clinovix\Security\AuditLog')) {
                \Clinovix\Security\AuditLog::record('security_lockout', ['object' => $context, 'severity' => 'critical', 'detail' => 'IP temporarily locked after repeated failures']);
            }
        } elseif (class_exists('\Clinovix\Security\AuditLog')) {
            \Clinovix\Security\AuditLog::record('auth_denied', ['object' => $context, 'severity' => 'warning', 'detail' => 'attempt ' . $count]);
        }
    }
}
