...
class SeoAnalyzer
{
    ...
    {
        return \Clinovix\Core\Cache::remember('seo_q_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            global $wpdb;
    ...
    {
        return \Clinovix\Core\Cache::remember('seo_kw_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            $counts = [];
    ...
    {
        return \Clinovix\Core\Cache::remember('seo_topics_' . $days, 600, function () use ($days) {
            $services = array_map(
                static fn($s) => (string) $s['name'],
                (new \Clinovix\Ai\SystemPromptBuilder(new \Clinovix\Core\Settings()))->services()
            );
            if ($services === []) {
                return [];
            }
