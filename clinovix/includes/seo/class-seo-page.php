...
class SeoPage
{
    private const NONCE = 'clx_seo';

    public function register(): void
    {
        add_action('admin_menu', [$this, 'menu'], 20);
        add_action('wp_ajax_clx_seo_generate', [$this, 'ajax_generate']);
    }
    ...
    public function render(): void
    {
        ...
        $analyzer  = new \Clinovix\Seo\SeoAnalyzer();
        $questions = $analyzer->top_questions(30, 15);
        $keywords  = $analyzer->keywords(30, 30);
        $topics    = $analyzer->topics(30);
        $nonce     = wp_create_nonce(self::NONCE);
        $cached    = get_transient('clx_seo_ideas');

        include CLX_DIR . 'includes/admin/views/seo.php';
    }
    ...
    public function ajax_generate(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Clinovix\Security\Security::note_failure('seo_generate');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        // Protect the API budget: a few generations per hour is plenty.
        if (! \Clinovix\Security\Security::rate_limit('seo_generate', 10, HOUR_IN_SECONDS)) {
            wp_send_json(['ok' => false, 'error' => __('محدودیت درخواست. کمی بعد دوباره تلاش کنید.', 'clinovix')], 429);
        }
        ...
        set_transient('clx_seo_ideas', $result['ideas'], 6 * HOUR_IN_SECONDS);
        \Clinovix\Security\AuditLog::record('seo_generate', ['object' => 'ideas']);
    ...
    private function generate(): array
    {
        ...
        $provider = (new \Clinovix\Ai\ProviderFactory($settings))->create_active();
        if ($provider === null) {
            return ['ok' => false, 'error' => __('کلید API تنظیم نشده است.', 'clinovix')];
        }
        ...
        if ($questions === [] && $keywords === []) {
            return ['ok' => false, 'error' => __('هنوز داده‌ی گفتگوی کافی برای تحلیل وجود ندارد.', 'clinovix')];
        }
        ...
        if (empty($result['ok'])) {
            return ['ok' => false, 'error' => __('خطا در ارتباط با هوش مصنوعی.', 'clinovix')];
        }
