...
# رگرسیون رفع‌شده پس از استقرار — نسخه ۱.۰.۲ (قالب)

## signteb-medcore/inc/class-medcore-template-tags.php — خط ۳۲۰
- ❌ مشکل: کامنت توضیحی‌ای که در ممیزی نسخه ۱.۰.۱ برای مستندسازی راهکار شورت‌کد اضافه شده بود، به‌صورت تحت‌اللفظی شامل دنباله‌ی بستن تگ PHP (`?>`) داخل یک کامنت تک‌خطی `//` بود. در PHP این دنباله **حتی داخل کامنت `//` هم بلوک PHP را می‌بندد**؛ در نتیجه از همان خط به بعد، کل ادامه‌ی فایل — از جمله بلوک `add_action('init', …)` که سه شورت‌کد (`stmc_breadcrumbs`, `stmc_whatsapp_float`, `stmc_copyright_year`) را ثبت می‌کرد — به‌عنوان کد اجرا نمی‌شد و به‌صورت **متن خام** روی هر صفحه‌ی سایت چاپ می‌شد. چون این فایل در بارگذاری اولیه‌ی قالب (پیش از ارسال هدرها) require می‌شود، این خروجی خام قبل از `<!DOCTYPE html>` تزریق شده و کل چیدمان/طراحی صفحه را هم به‌هم می‌ریخت. علاوه بر آن، عدم ثبت شورت‌کدها باعث می‌شد کپی‌رایت فوتر، دکمه شناور واتساپ و breadcrumbs کار نکنند.
- 🔍 دلیل: `php -l` این را نمی‌گیرد چون بستن `?>` و متن پس از آن از نظر سینتکسی کاملاً معتبر است (یک باگ معنایی، نه سینتکسی). عامل انسانی/ابزاری: نوشتن یک نمونه‌ی «`<?php ?>`» به‌صورت literal داخل متن کامنت.
- ✅ اصلاح: متن کامنت بازنویسی شد تا هیچ دنباله‌ی literal بستن تگ PHP نداشته باشد («تگ‌های باز/بسته‌ی PHP» به‌جای نماد literal). با یک هارنس تست، تأیید شد که فایل اکنون بدون هیچ خروجی خام کاملاً اجرا می‌شود و هر سه شورت‌کد درست ثبت می‌شوند (`[stmc_copyright_year]` → 2026).
- ⚠️ ریسک قبل از اصلاح: Critical (کل فرانت سایت — طراحی و محتوا — خراب می‌شد)
