...
jest.mock( '@wordpress/api-fetch', () => {
	const mock = ( ...args: unknown[] ) => mockApiFetch( ...args );
	mock.use = jest.fn();
...
describe( 'request', () => {
	...
	it( 'does not double up slashes', async () => {
		...
		expect( mockApiFetch.mock.calls[ 0 ][ 0 ].url ).toBe(
	...
	it( 'appends query parameters', async () => {
		...
		const url = mockApiFetch.mock.calls[ 0 ][ 0 ].url as string;
	...
	it( 'encodes parameters that need it', async () => {
		...
		// The `&` must not survive raw, or it splits the query into two
		// parameters and the search silently loses everything after it.
		expect( url ).not.toContain( ' & more' );

		// Read it back the way a query-string parser does: form encoding
		// writes a space as `+`, so `decodeURIComponent` alone would not
		// round-trip it.
		const parsed = new URLSearchParams( url.split( '?' )[ 1 ] );

		expect( parsed.get( 'q' ) ).toBe( 'کبد چرب & more' );
	} );
} );
