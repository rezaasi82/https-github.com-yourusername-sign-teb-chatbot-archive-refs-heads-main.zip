/**
 * Medora — frontend widget (Vanilla JS, no jQuery).
 *
...
(function () {
	...
	if (typeof window.MDR_CONFIG === 'undefined') {
		return;
	}

	var cfg = window.MDR_CONFIG;
	...
	var roots = document.querySelectorAll('.mdr-root');
	...
	var panel = root.querySelector('.mdr-panel');
	var launcher = root.querySelector('.mdr-launcher');
	var closeBtn = root.querySelector('.mdr-close');
	var form = root.querySelector('.mdr-form');
	var input = root.querySelector('.mdr-input');
	var messages = root.querySelector('.mdr-messages');
	var quickWrap = root.querySelector('.mdr-quick');
	var lead = root.querySelector('.mdr-lead');
	...
	// Opt-in debug logging: enable with localStorage.setItem('mdr_debug','1')
	// or by setting MDR_CONFIG.debug = true. Traces the full flow so the source
	// of any issue is visible in the console.
	var DEBUG = !!cfg.debug || (function () {
		try { return window.localStorage.getItem('mdr_debug') === '1'; } catch (e) { return false; }
	})();
	function log() {
	...
	function getSession() {
		var existing = load('mdr_session');
		if (existing) { return existing; }
		var id = 'sess_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
		store('mdr_session', id);
		return id;
	}

	function getProfile() {
		try { return JSON.parse(load('mdr_profile') || '{}') || {}; } catch (e) { return {}; }
	}
	...
	function lockBody() {
		if (!isMobile() || document.body.classList.contains('mdr-body-lock')) { return; }
		savedScrollY = window.scrollY || window.pageYOffset || 0;
		document.body.style.top = '-' + savedScrollY + 'px';
		document.body.classList.add('mdr-body-lock');
	}
	function unlockBody() {
		if (!document.body.classList.contains('mdr-body-lock')) { return; }
		document.body.classList.remove('mdr-body-lock');
	...
	function closePanel() {
		root.classList.remove('mdr-open');
		panel.hidden = true;
		panel.style.height = '';
		panel.style.transform = '';
		unlockBody();
	...
	function maybeShowLead() {
		...
		if (lead) {
			lead.hidden = false;
			root.classList.add('mdr-lead-active');
			var nameField = lead.querySelector('.mdr-lead-name');
	...
	if (lead) {
		var startBtn = lead.querySelector('.mdr-lead-start');
		var skipBtn = lead.querySelector('.mdr-lead-skip');
		var errBox = lead.querySelector('.mdr-lead-error');

		startBtn.addEventListener('click', function () {
			var name = (lead.querySelector('.mdr-lead-name').value || '').trim();
			var phone = (lead.querySelector('.mdr-lead-phone').value || '').trim();
	...
	function appendMessage(text, who) {
		...
		el.className = 'mdr-msg ' + (who === 'user' ? 'mdr-msg-user' : 'mdr-msg-bot');
	...
	function showTyping() {
		var el = document.createElement('div');
		el.className = 'mdr-msg mdr-msg-bot mdr-typing';
	...
	function renderCtaCard(card) {
		...
		wrap.className = 'mdr-cta-card';

		if (root.dataset.chBooking === '1' && bookingUrl) {
			var head = document.createElement('div');
			head.className = 'mdr-cta-head';
			head.innerHTML = '<div class="mdr-cta-title"></div><div class="mdr-cta-text"></div>';
			head.querySelector('.mdr-cta-title').textContent = cfg.strings.ctaTitle;
			head.querySelector('.mdr-cta-text').textContent = cfg.strings.ctaText;
			wrap.appendChild(head);
			wrap.appendChild(channelBtn(cfg.strings.book, bookingUrl, 'booking', '📅', true));
		}

		var row = document.createElement('div');
		row.className = 'mdr-channel-row';
	...
	function channelBtn(label, href, type, emoji, primary) {
		...
		a.className = 'mdr-cta-btn mdr-cta-' + type + (primary ? ' mdr-cta-primary' : '');
		a.href = href;
		a.target = '_blank';
		a.rel = 'noopener';
		a.innerHTML = '<span class="mdr-cta-emoji" aria-hidden="true"></span><span class="mdr-cta-label"></span>';
		a.querySelector('.mdr-cta-emoji').textContent = emoji;
		a.querySelector('.mdr-cta-label').textContent = label;
	...
	function ajaxFallback(text) {
		...
		var p = payload(text);
		body.append('action', 'mdr_chat_message');
	...
	function trackEvent(type) {
		...
		fetch(cfg.eventUrl, {
			...
			var body = new URLSearchParams();
			body.append('action', 'mdr_track_event');
			body.append('nonce', cfg.ajaxNonce);
	...
	if (quickWrap) {
		quickWrap.addEventListener('click', function (e) {
			var btn = e.target.closest('.mdr-quick-reply');
	...
	(function initTeaser() {
		...
		function reveal() {
			...
			teaser.hidden = false;
			root.classList.add('mdr-teaser-on');
		...
		function dismiss(persist) {
			clearTimeout(timer);
			teaser.hidden = true;
			root.classList.remove('mdr-teaser-on');
			if (persist) { store('mdr_teaser_dismissed', '1'); }
		}

		var closeBubble = teaser.querySelector('.mdr-teaser-close');
		if (closeBubble) {
