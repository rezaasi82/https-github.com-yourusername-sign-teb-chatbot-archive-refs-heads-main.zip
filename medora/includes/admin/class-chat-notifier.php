 ...
 *   3. a count badge on the Medora menu item (same style as Comments).
 *
...
class ChatNotifier
{
    private const META = 'mdr_chats_seen_at';

    /** Conversations list URL — the click target everywhere. */
    public static function list_url(): string
    {
        return admin_url('admin.php?page=mdr-chat&tab=conversations');
    }
    ...
    public function maybe_mark_seen(): void
    {
        ...
        if ($page === 'mdr-chat' && $tab === 'conversations') {
            $this->mark_seen();
        }
    }
    ...
    public function admin_bar(\WP_Admin_Bar $bar): void
    {
        ...
        $label = sprintf(
            ...
            __('%s گفتگوی جدید', 'medora'),
            '<span class="mdr-ab-count">' . esc_html(number_format_i18n($count)) . '</span>'
        );
        $bar->add_node([
            'id'    => 'mdr-chats',
            'title' => '<span class="mdr-ab-ico" aria-hidden="true">💬</span> ' . $label,
            'href'  => self::list_url(),
            'meta'  => [
                'class' => $count > 0 ? 'mdr-ab-has-new' : 'mdr-ab-zero',
                'title' => __('گفتگوهای جدید Medora', 'medora'),
            ],
        ]);
    }
    ...
    public function dashboard_notice(): void
    {
        ...
        printf(
            '<div class="notice notice-info mdr-new-chats-notice"><p><strong>%s</strong> %s <a class="button button-primary" href="%s">%s</a></p></div>',
            esc_html__('Medora:', 'medora'),
            esc_html(sprintf(
                /* translators: %s: number of new conversations */
                _n('%s گفتگوی جدید از آخرین بازدید شما ثبت شده است.', '%s گفتگوی جدید از آخرین بازدید شما ثبت شده است.', $count, 'medora'),
                number_format_i18n($count)
            )),
            esc_url(self::list_url()),
            esc_html__('مشاهده گفتگوها', 'medora')
        );
    }
    ...
    public function enqueue(): void
    {
        ...
        wp_enqueue_script(
            'mdr-notifier',
            MDR_URL . 'assets/js/notifier.js',
            ['jquery', 'heartbeat'],
            MDR_VERSION,
            true
        );
        $css = '
            #wp-admin-bar-mdr-chats.mdr-ab-zero { display: none; }
            #wp-admin-bar-mdr-chats .mdr-ab-count {
            ...
            #wp-admin-bar-mdr-chats .mdr-ab-ico { margin-inline-end: 2px; }
        ';
        wp_register_style('mdr-notifier', false, [], MDR_VERSION);
        wp_enqueue_style('mdr-notifier');
        wp_add_inline_style('mdr-notifier', $css);
    }
    ...
    {
        if (! empty($data['mdr_notify']) && current_user_can('manage_options')) {
            $response['mdr_new_chats'] = $this->unseen_count();
        }
        return $response;
    }
}
