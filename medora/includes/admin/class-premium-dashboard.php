...
{
    private const PAGE  = 'mdr-dashboard';
    private const NONCE = 'mdr_dashboard';

    public function register(): void
    {
        add_action('admin_menu', [$this, 'menu'], 20);
        add_action('admin_enqueue_scripts', [$this, 'enqueue']);
        add_action('wp_ajax_mdr_integrity_check', [$this, 'ajax_integrity']);
    }
    ...
    public function enqueue(string $hook): void
    {
        ...
        wp_enqueue_style('mdr-dashboard', MDR_URL . 'assets/css/dashboard.css', [], MDR_VERSION);
        wp_enqueue_script('mdr-dashboard', MDR_URL . 'assets/js/dashboard.js', [], MDR_VERSION, true);
        wp_localize_script('mdr-dashboard', 'MDR_DASH', [
    ...
    public function render(): void
    {
        ...
        include MDR_DIR . 'includes/admin/views/premium-dashboard.php';
    }
    ...
    {
        $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('نامعتبر', 'medora'), 'code' => 0];

        try {
            $env_ok = defined('MDR_VERSION')
                && defined('MDR_FILE')
                && class_exists('\Medora\Core\Plugin');

            if ($env_ok === true) {
                $verdict = ['ok' => true, 'level' => 'secure', 'label' => __('فعال و ایمن', 'medora'), 'code' => 200];
            }
        } catch (\Throwable $e) {
            $this->log_anomaly('integrity_gate', $e);
            $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('خطای بررسی', 'medora'), 'code' => 500];
        }
