...
class SettingsPage
{
    ...
    {
        if (! \Medora\Core\Input::has_post('mdr_settings_submit')) {
            return;
        }
        if (! current_user_can('manage_options')) {
            return;
        }
        check_admin_referer('mdr_settings');
    ...
    private function finish(string $tab): void
    {
        ...
        add_settings_error('pzr', 'saved', __('تنظیمات ذخیره شد.', 'medora'), 'updated');
        set_transient('settings_errors', get_settings_errors(), 30);
        wp_safe_redirect(admin_url('admin.php?page=mdr-chat&tab=' . $tab . '&updated=1'));
        exit;
    }
    ...
    public function render(): void
    {
        ...
        echo '<div class="wrap mdr-admin" dir="rtl">';
        ...
        if ($float && $shortcode) {
            $display = __('ویجت شناور + شورت‌کد', 'medora');
        } elseif ($float) {
            $display = __('فقط ویجت شناور', 'medora');
        } elseif ($shortcode) {
            $display = __('فقط شورت‌کد', 'medora');
        } else {
            $display = __('نمایش خاموش', 'medora');
        }

        \Medora\Admin\PageHeader::render(
            __('Medora AI', 'medora'),
            __('دستیار هوشمند جذب بیمار', 'medora'),
            [
                [
                    'label' => $display,
                ...
                [
                    'label' => $has_key
                        ? __('کلید API تنظیم شده', 'medora')
                        : __('کلید API تنظیم نشده', 'medora'),
                    'state' => $has_key ? 'on' : 'off',
                ],
                ['label' => __('نسخه', 'medora') . ' ' . MDR_VERSION],
            ]
        );

        // Pistachio-green success toast after a save (auto-dismisses via CSS).
        if (\Medora\Core\Input::get_key('updated') === '1') {
            echo '<div class="mdr-saved-toast" role="status">'
                . '<span class="mdr-saved-ico" aria-hidden="true">✓</span>'
                . '<span>' . esc_html__('تغییرات با موفقیت ذخیره شد.', 'medora') . '</span>'
                . '</div>';
        } else {
            settings_errors('pzr');
        }
        ...
        if (in_array($tab, ['provider', 'clinic', 'appearance', 'integrations'], true)) {
            include MDR_DIR . 'includes/admin/views/settings.php';
    ...
    {
        $labels = [
            'provider'      => __('هوش مصنوعی', 'medora'),
            'clinic'        => __('اطلاعات کلینیک', 'medora'),
            'appearance'    => __('ظاهر ویجت', 'medora'),
            'integrations'  => __('اتصال‌ها و خروجی', 'medora'),
            'conversations' => __('لیدها و مکالمات', 'medora'),
            'stats'         => __('آمار', 'medora'),
        ];
        echo '<h2 class="nav-tab-wrapper">';
        foreach ($labels as $slug => $label) {
            $url    = admin_url('admin.php?page=mdr-chat&tab=' . $slug);
