...
class AiManager
{
    ...
    public function handle(array $req): array
    {
        if (! $this->settings->is_enabled()) {
            return ['ok' => false, 'code' => 'disabled', 'error' => __('چت‌بات غیرفعال است.', 'medora')];
        }

        $message = trim((string) $req['message']);
        if ($message === '') {
            return ['ok' => false, 'code' => 'empty', 'error' => __('پیام خالی است.', 'medora')];
        }
        ...
        if (! $limiter->allow($req['ip'] . '|' . $req['session_id'])) {
            return ['ok' => false, 'code' => 'rate_limited', 'error' => __('لطفاً کمی صبر کنید و دوباره تلاش کنید.', 'medora')];
        }
        ...
        $context = [
            ...
            'max_tokens'  => 1200,
            'temperature' => (float) apply_filters('mdr_temperature', 0.8),
        ];
        ...
        if ($cta !== '') {
            ...
            do_action('mdr_lead_detected', $conversation_id, $cta);
        }
        ...
        do_action('mdr_message_handled', $conversation_id, $cta, $score['level']);
