...
class LeadCrm
{
    ...
    {
        add_action('wp_ajax_mdr_lead_update', [$this, 'ajax_update']);
        add_action('wp_ajax_mdr_lead_refer', [$this, 'ajax_refer']);
    }
    ...
    public static function referral_text(object $c): string
    {
        $scores = [
            'hot'  => __('داغ', 'medora'),
            'warm' => __('متوسط', 'medora'),
            'cold' => __('سرد', 'medora'),
        ];
        $lines = [
            __('ارجاع لید از Medora', 'medora'),
            sprintf(__('نام: %s', 'medora'), trim((string) ($c->patient_name ?? '')) ?: '—'),
            sprintf(__('موبایل: %s', 'medora'), trim((string) ($c->patient_phone ?? '')) ?: '—'),
            sprintf(__('امتیاز: %s', 'medora'), $scores[$c->lead_score ?? ''] ?? '—'),
            sprintf(__('وضعیت: %s', 'medora'), self::label((string) ($c->lead_status ?? 'new'))),
        ];
        $summary = trim((string) ($c->summary ?? ''));
        if ($summary !== '') {
            $lines[] = "\n" . __('خلاصه گفتگو:', 'medora') . "\n" . $summary;
        }
        return implode("\n", $lines);
    }
    ...
    public function ajax_refer(): void
    {
        ...
        if (! is_email($to)) {
            wp_send_json(['ok' => false, 'error' => __('ایمیل مقصد نامعتبر است.', 'medora')], 400);
        }
        ...
        $subject = sprintf(__('[%s] ارجاع لید — %s', 'medora'), $clinic, trim((string) ($c->patient_name ?? '')) ?: ('#' . $lead_id));
        $body    = self::referral_text($c) . "\n\n" . admin_url('admin.php?page=mdr-chat&tab=conversations&conversation=' . $lead_id);

        $sent = wp_mail($to, $subject, $body);
        \Medora\Security\AuditLog::record('lead_refer', ['object' => (string) $lead_id, 'severity' => $sent ? 'info' : 'warning']);

        if (! $sent) {
            wp_send_json(['ok' => false, 'error' => __('ارسال ایمیل ناموفق بود. تنظیمات ایمیل سایت را بررسی کنید.', 'medora')], 500);
        }
    ...
    public function ajax_update(): void
    {
        ...
        do_action('mdr_lead_crm_updated', $lead_id, $fields);
