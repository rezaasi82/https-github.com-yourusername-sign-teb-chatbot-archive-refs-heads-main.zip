 ...
 * under uploads/medora-pdf/YYYY/MM/ with an unguessable name.
 *
 * @package Medora
 */
...
class PdfGenerator
{
    ...
    private function target_dir(): ?array
    {
        ...
        $sub  = '/medora-pdf/' . gmdate('Y') . '/' . gmdate('m');
        $path = $uploads['basedir'] . $sub;
        if (! wp_mkdir_p($path)) {
            return null;
        }
        // Prevent directory listing.
        if (! file_exists($uploads['basedir'] . '/medora-pdf/index.php')) {
            file_put_contents($uploads['basedir'] . '/medora-pdf/index.php', "<?php\n// Silence is golden.\n");
        }
    ...
    public function render_html(array $payload): string
    {
        ...
        foreach ($payload['messages'] as $m) {
            $is_user = $m['role'] === 'user';
            $who     = $is_user ? 'بیمار' : 'دستیار Medora';
            $side    = $is_user ? 'mdr-p' : 'mdr-a';
...
<style>
    * { box-sizing: border-box; }
    ...
    .mdr-p { background: #eef2fb; border: 1px solid #dbe2f2; margin-inline-start: auto; }
    .mdr-a { background: #fff; border: 1px solid #e6e9f0; }
...
<div class="report">
    ...
    <div class="foot">
        <span>Medora · <?php echo esc_html($exported); ?></span>
