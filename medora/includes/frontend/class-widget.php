...
class Widget
{
    ...
    private function should_render(): bool
    {
        ...
        return (bool) apply_filters('mdr_should_render', true);
    }
    ...
    {
        if (wp_script_is('mdr-widget', 'enqueued')) {
            return;
        }

        // Optional self-hosted Vazirmatn font (only if the file is bundled).
        if ((int) $this->settings->get('use_bundled_font', 1) === 1
            && file_exists(MDR_DIR . 'assets/fonts/vazirmatn.woff2')) {
            wp_enqueue_style('mdr-font', MDR_URL . 'assets/fonts/font.css', [], MDR_VERSION);
        }

        wp_enqueue_style('mdr-widget', MDR_URL . 'assets/css/widget.css', [], MDR_VERSION);
        wp_enqueue_script('mdr-widget', MDR_URL . 'assets/js/widget.js', [], MDR_VERSION, true);

        wp_localize_script('mdr-widget', 'MDR_CONFIG', [
            'restUrl'   => esc_url_raw(rest_url('medora/v1/message')),
            'eventUrl'  => esc_url_raw(rest_url('medora/v1/event')),
            'restNonce' => wp_create_nonce('wp_rest'),
            'ajaxUrl'   => esc_url_raw(admin_url('admin-ajax.php')),
            'ajaxNonce' => wp_create_nonce('mdr_chat_nonce'),
            'pageUrl'   => \Medora\Core\Input::request_uri(),
            'strings'   => [
                'placeholder' => __('پیام خود را بنویسید…', 'medora'),
                'send'        => __('ارسال', 'medora'),
                'typing'      => __('در حال نوشتن…', 'medora'),
                'book'        => __('رزرو نوبت آنلاین', 'medora'),
                'whatsapp'    => __('واتساپ', 'medora'),
                'call'        => __('تماس با مطب', 'medora'),
                'bale'        => __('ارتباط در بله', 'medora'),
                'error'       => __('خطا در ارتباط. دوباره تلاش کنید.', 'medora'),
                'ctaTitle'    => __('آماده دریافت نوبت هستید؟', 'medora'),
                'ctaText'     => __('برای رزرو آنلاین و انتخاب زمان مراجعه روی دکمه زیر کلیک کنید.', 'medora'),
            ],
        ]);
    }
    ...
    public function render(): void
    {
        ...
        include MDR_DIR . 'templates/widget.php';
    }
    ...
    private function build_config(bool $inline): array
    {
        ...
        return [
            ...
            'bot_name'      => (string) $s->get('bot_name', __('دستیار هوشمند', 'medora')),
            ...
            'branch'        => (int) apply_filters('mdr_widget_branch', (int) $s->get('default_branch', 0)),
