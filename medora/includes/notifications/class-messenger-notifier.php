...
class MessengerNotifier
{
    ...
    private const CHANNELS = [
        'bale'     => ['https://tapi.bale.ai', 'mdr_msgr_bale_token_enc', 'بله'],
        'telegram' => ['https://api.telegram.org', 'mdr_msgr_tg_token_enc', 'Telegram'],
    ];
    ...
    {
        add_action('mdr_lead_detected', [$this, 'notify_new_lead'], 10, 2);
        add_action('wp_ajax_mdr_test_messenger', [$this, 'ajax_test']);
    }
    ...
    public function send(string $channel, string $text): array
    {
        ...
        if ($token === '' || $chat === '') {
            return ['ok' => false, 'error' => __('توکن ربات یا شناسه چت تنظیم نشده است.', 'medora')];
        }
        ...
        return ['ok' => false, 'code' => $code, 'error' => $desc !== '' ? $desc : sprintf(__('پاسخ سرویس: HTTP %d', 'medora'), $code)];
    }
    ...
    private function lead_text(object $c, string $cta): string
    {
        ...
        $clinic = $vars['clinic'];
        $lines  = [
            '🔔 ' . sprintf(__('لید جدید در %s', 'medora'), $clinic),
            sprintf(__('نام: %s', 'medora'), $vars['name']),
            sprintf(__('موبایل: %s', 'medora'), $vars['phone'] !== '' ? $vars['phone'] : '—'),
            sprintf(__('امتیاز: %s', 'medora'), $vars['score']),
        ];
        if ($cta !== '') {
            $lines[] = sprintf(__('اقدام: %s', 'medora'), $cta);
        }
        if ($vars['summary'] !== '') {
            $lines[] = "\n" . $vars['summary'];
        }
    ...
    public function ajax_test(): void
    {
        ...
        wp_send_json($this->send($channel, sprintf(__('پیام آزمایشی %s از Medora ✅', 'medora'), $clinic)));
    }
}
