...
class SmsManager
{
    public const OPTION_KEY    = 'mdr_sms_key_enc';
    public const OPTION_SECRET = 'mdr_sms_secret_enc';
    ...
    public function send(string $to, string $text): array
    {
        $text = trim($text);
        if ($text === '') {
            return ['ok' => false, 'error' => __('متن پیام خالی است.', 'medora')];
        }
    ...
    private function dispatch(callable $call): array
    {
        if (! $this->is_configured()) {
            return ['ok' => false, 'error' => __('پنل پیامک پیکربندی نشده است.', 'medora')];
        }
        $provider = $this->active();
        if ($provider === null) {
            return ['ok' => false, 'error' => __('سرویس پیامک نامعتبر است.', 'medora')];
        }
    ...
    {
        $lines   = [];
        $lines[] = sprintf(__('سرویس انتخاب‌شده: %s', 'medora'), $this->providers()[$this->active_id()] ?? $this->active_id());
        $lines[] = sprintf(__('فعال‌سازی: %s', 'medora'), $this->is_enabled() ? '✓' : __('✗ (تیک «فعال‌سازی» را بزنید)', 'medora'));
        $lines[] = sprintf(__('APIKey ذخیره شده: %s', 'medora'), self::key() !== '' ? '✓' : '✗');
        $lines[] = sprintf(__('رمز عبور ذخیره شده: %s', 'medora'), self::secret() !== '' ? __('✓ (حالت نام‌کاربری/رمز)', 'medora') : __('— (حالت توکن)', 'medora'));
        $lines[] = sprintf(__('شماره فرستنده: %s', 'medora'), trim((string) $this->settings->get('sms_sender', '')) !== '' ? $this->settings->get('sms_sender') : __('خالی (برای خط اشتراکی درست است)', 'medora'));

        $codes = array_filter(array_map([$this, 'template_code'], array_keys(self::default_templates())));
        $lines[] = sprintf(__('کد الگو تنظیم‌شده: %d قالب', 'medora'), count($codes));
        ...
        $lines[] = __('این سرویس بررسی زنده ندارد؛ با «ارسال پیامک تست» امتحان کنید.', 'medora');
    ...
    public static function default_templates(): array
    {
        ...
        return [
            'welcome'  => [
                'label' => __('خوش‌آمد به لید', 'medora'),
                'text'  => __('{0} عزیز، از تماس شما با {1} سپاسگزاریم. کارشناسان ما به‌زودی برای هماهنگی با شما تماس می‌گیرند. 🌿 {optout}', 'medora'),
                'vars'  => ['name', 'clinic'],
            ],
            'referral' => [
                'label' => __('ارجاع لید به همکار', 'medora'),
                'text'  => __('لید جدید در سایت : نام {0} -شماره {1} - امتیاز: {2}. لطفاً پیگیری کنید. {optout}', 'medora'),
                'vars'  => ['name', 'phone', 'score'],
            ],
            'reminder' => [
                'label' => __('یادآوری پیگیری', 'medora'),
                'text'  => __('{0} عزیز، جهت تکمیل مشاوره و رزرو نوبت با {1} در ارتباط باشید. منتظر شما هستیم. {optout}', 'medora'),
                'vars'  => ['name', 'clinic'],
            ],
            'custom'   => [
                'label' => __('پیام سفارشی', 'medora'),
                'text'  => __('{0} عزیز، {1} در خدمت شماست. {optout}', 'medora'),
                'vars'  => ['name', 'clinic'],
            ],
        ];
    }
    ...
    {
        $scores = ['hot' => __('داغ', 'medora'), 'warm' => __('متوسط', 'medora'), 'cold' => __('سرد', 'medora')];
        return [
            'name'    => trim((string) ($c->patient_name ?? '')) ?: __('کاربر', 'medora'),
    ...
    public static function messenger_links(string $phone, string $text): array
    {
        ...
        return [
            'sms'      => ['label' => __('پیامک دستگاه', 'medora'), 'url' => 'sms:' . $digits . '?&body=' . $enc],
