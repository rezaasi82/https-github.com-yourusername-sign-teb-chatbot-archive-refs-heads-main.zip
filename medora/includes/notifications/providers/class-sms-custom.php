...
class SmsCustom extends \Medora\Notifications\SmsProviderBase
{
    ...
    {
        return __('سفارشی / خارجی (Custom HTTP)', 'medora');
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($url === '') {
            return ['ok' => false, 'error' => __('آدرس سرویس سفارشی تنظیم نشده است.', 'medora')];
        }
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $r['error'] ?? sprintf(__('پاسخ سرویس: HTTP %d', 'medora'), $r['code'])];
    }
