...
- **صفحه ورود سفارشی VIP (`LoginCustomizer`)**: بازطراحی کامل `wp-login.php` با تم تیره الهام‌گرفته از GitHub Dark (`#0d1117`/`#161b22`)، لهجه‌های طلایی VIP، نشان VIP زیر لوگو، لوگوی تقویم، پیام خوش‌آمد فارسی و فیلدهای مدرن — استایل فقط در صفحه ورود لود می‌شود (سازگار با قانون conditional asset loading)
- **ایکون‌های شناور تزئینی** در پس‌زمینه صفحه ورود (تقویم، ساعت، زنگ، ستاره، تیک، بلیت) با انیمیشن ملایم — غیرقابل کلیک، `aria-hidden` و ثابت در حالت `prefers-reduced-motion`
- قابل غیرفعال‌سازی با آپشن `nobatyar_login_customizer_enabled` یا فیلتر همنام؛ فرم re-auth داخل iframe (interim login) دست‌نخورده می‌ماند
