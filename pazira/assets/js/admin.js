/**
 * Pazira AI Web Chat — admin settings (Vanilla JS).
...
(function () {
	...
	var select = document.getElementById('pzr-provider');
	if (!select) {
		return;
	}

	var blocks = document.querySelectorAll('.pzr-provider-block');
...
(function () {
	'use strict';
	if (typeof window.PZR_ADMIN === 'undefined') { return; }
	var A = window.PZR_ADMIN;
	...
	document.querySelectorAll('.pzr-act').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var op = btn.getAttribute('data-op');
			var lead = btn.getAttribute('data-lead');
			var original = btn.textContent;
			btn.disabled = true; btn.textContent = A.strings.working;
			var action = op === 'webhook' ? 'pzr_export_webhook' : 'pzr_export_gsheet';
	...
	var checkAll = document.getElementById('pzr-check-all');
	if (checkAll) {
		checkAll.addEventListener('change', function () {
			document.querySelectorAll('.pzr-check').forEach(function (c) { c.checked = checkAll.checked; });
		});
	}
	var bulkApply = document.getElementById('pzr-bulk-apply');
	if (bulkApply) {
		bulkApply.addEventListener('click', function () {
			var op = document.getElementById('pzr-bulk-op').value;
			var ids = [];
			document.querySelectorAll('.pzr-check:checked').forEach(function (c) { ids.push(c.value); });
			var out = document.getElementById('pzr-bulk-result');
			if (!op || ids.length === 0) { out.textContent = A.strings.noSel; return; }
			bulkApply.disabled = true; out.textContent = A.strings.working;
			post('pzr_export_bulk', { op: op, ids: ids }).then(function (res) {
				if (res && res.ok) {
	...
	var crm = document.querySelector('.pzr-crm-panel');
	if (crm) {
		var saveBtn = crm.querySelector('.pzr-crm-save');
		var result = crm.querySelector('.pzr-crm-result');
		saveBtn.addEventListener('click', function () {
			saveBtn.disabled = true; result.textContent = A.strings.working;
			var branchSel = crm.querySelector('.pzr-crm-branch');
			var data = {
				lead_id: crm.getAttribute('data-lead'),
				nonce: crm.getAttribute('data-nonce'),
				lead_status: crm.querySelector('.pzr-crm-status').value,
				email: crm.querySelector('.pzr-crm-email').value,
				tags: crm.querySelector('.pzr-crm-tags').value,
				notes: crm.querySelector('.pzr-crm-notes').value
			};
			if (branchSel) { data.branch_id = branchSel.value; }
			post('pzr_lead_update', data).then(function (res) {
	...
	var smsProvider = document.querySelector('.pzr-sms-provider');
	if (smsProvider) {
		var customBox = document.querySelector('.pzr-sms-custom');
		var secretRow = document.querySelector('.pzr-sms-secret-row');
		var meliHint = document.querySelector('.pzr-sms-hint[data-for="melipayamak"]');
	...
	var smsDiagBtn = document.querySelector('.pzr-sms-diag-btn');
	if (smsDiagBtn) {
		smsDiagBtn.addEventListener('click', function () {
			var out = document.querySelector('.pzr-sms-diag-out');
			smsDiagBtn.disabled = true;
			if (out) { out.style.display = ''; out.textContent = A.strings.working; }
			post('pzr_sms_diag', {}).then(function (res) {
				if (out) {
	...
	var smsTestBtn = document.querySelector('.pzr-sms-test-btn');
	if (smsTestBtn) {
		smsTestBtn.addEventListener('click', function () {
			var toEl = document.querySelector('.pzr-sms-test-to');
			var out = document.querySelector('.pzr-test-result[data-for="sms"]');
			var to = toEl ? toEl.value : '';
			if (!to) { if (out) { out.textContent = A.strings.noSel; } return; }
			smsTestBtn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post('pzr_test_sms', { to: to }).then(function (res) {
				if (out) {
	...
	document.querySelectorAll('.pzr-msgr-test-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var ch = btn.getAttribute('data-channel');
			var out = document.querySelector('.pzr-test-result[data-for="msgr-' + ch + '"]');
			btn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post('pzr_test_messenger', { channel: ch }).then(function (res) {
				if (out) {
					out.textContent = (res && res.ok) ? ('✓ ' + A.strings.ok) : ('✕ ' + ((res && res.error) || A.strings.failed));
					out.style.color = (res && res.ok) ? '#1a7f37' : '#d63638';
				}
	...
	var refer = document.querySelector('.pzr-refer');
	if (refer) {
		var refText = refer.getAttribute('data-text') || '';
		var refResult = refer.querySelector('.pzr-refer-result');
		var origResult = refResult ? refResult.textContent : '';

		var mailBtn = refer.querySelector('.pzr-refer-mail');
		if (mailBtn) {
			mailBtn.addEventListener('click', function () {
				var to = (refer.querySelector('.pzr-refer-email') || {}).value || '';
				if (!to) { refResult.textContent = A.strings.noSel; return; }
				mailBtn.disabled = true; refResult.textContent = A.strings.working;
				post('pzr_lead_refer', { lead_id: refer.getAttribute('data-lead'), to: to }).then(function (res) {
		...
		var staffSel = refer.querySelector('.pzr-refer-staff');
		var phoneField = refer.querySelector('.pzr-refer-phone');
		var emailField = refer.querySelector('.pzr-refer-email');
		if (staffSel) {
		...
		var panelBtn = refer.querySelector('.pzr-refer-panel');
		if (panelBtn) {
			panelBtn.addEventListener('click', function () {
				var phone = (refer.querySelector('.pzr-refer-phone') || {}).value || '';
				if (!phone) { refResult.textContent = A.strings.noSel; return; }
				var tpl = (refer.querySelector('.pzr-refer-template') || {}).value || 'referral';
				panelBtn.disabled = true; refResult.textContent = A.strings.working;
				post('pzr_send_sms', { lead_id: refer.getAttribute('data-lead'), to: phone, template: tpl }).then(function (res) {
					refResult.textContent = (res && res.ok) ? ('✓ ' + A.strings.ok) : ('✕ ' + ((res && res.error) || A.strings.failed));
					refResult.style.color = (res && res.ok) ? '#1a7f37' : '#d63638';
	...
	var seoGen = document.getElementById('pzr-seo-gen');
	if (seoGen) {
		seoGen.addEventListener('click', function () {
			var wrap = document.querySelector('.pzr-seo-ai');
			var out = document.getElementById('pzr-seo-ideas');
			seoGen.disabled = true;
			var original = seoGen.textContent;
			seoGen.textContent = A.strings.working;
			out.textContent = '…';
			post('pzr_seo_generate', { nonce: wrap.getAttribute('data-nonce') }).then(function (res) {
	...
	document.querySelectorAll('.pzr-test-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var target = btn.getAttribute('data-target');
			var out = document.querySelector('.pzr-test-result[data-for="' + target + '"]');
			btn.disabled = true; if (out) { out.textContent = A.strings.working; }
			post(target === 'webhook' ? 'pzr_test_webhook' : 'pzr_test_gsheet', {}).then(function (res) {
				if (out) {
