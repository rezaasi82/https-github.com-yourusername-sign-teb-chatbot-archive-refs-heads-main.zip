/**
 * Pazira — frontend widget (Vanilla JS, no jQuery).
 *
...
(function () {
	...
	if (typeof window.PZR_CONFIG === 'undefined') {
		return;
	}

	var cfg = window.PZR_CONFIG;

	// One page can host several widgets (the floating launcher plus any number
	// of [pazira_chat] shortcodes), so initialise every instance independently.
	var roots = document.querySelectorAll('.pzr-root');
	...
	var panel = root.querySelector('.pzr-panel');
	var launcher = root.querySelector('.pzr-launcher');
	var closeBtn = root.querySelector('.pzr-close');
	var form = root.querySelector('.pzr-form');
	var input = root.querySelector('.pzr-input');
	var messages = root.querySelector('.pzr-messages');
	var quickWrap = root.querySelector('.pzr-quick');
	var lead = root.querySelector('.pzr-lead');
	...
	// Opt-in debug logging: enable with localStorage.setItem('pzr_debug','1')
	// or by setting PZR_CONFIG.debug = true. Traces the full flow so the source
	// of any issue is visible in the console.
	var DEBUG = !!cfg.debug || (function () {
		try { return window.localStorage.getItem('pzr_debug') === '1'; } catch (e) { return false; }
	})();
	function log() {
		if (!DEBUG || !window.console) { return; }
		try { console.log.apply(console, ['[Pazira]'].concat([].slice.call(arguments))); } catch (e) {}
	}
	...
	function getSession() {
		var existing = load('pzr_session');
		if (existing) { return existing; }
		var id = 'sess_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
		store('pzr_session', id);
		return id;
	}

	function getProfile() {
		try { return JSON.parse(load('pzr_profile') || '{}') || {}; } catch (e) { return {}; }
	}
	...
	function lockBody() {
		if (!isMobile() || document.body.classList.contains('pzr-body-lock')) { return; }
		savedScrollY = window.scrollY || window.pageYOffset || 0;
		document.body.style.top = '-' + savedScrollY + 'px';
		document.body.classList.add('pzr-body-lock');
	}
	function unlockBody() {
		if (!document.body.classList.contains('pzr-body-lock')) { return; }
		document.body.classList.remove('pzr-body-lock');
	...
	function closePanel() {
		root.classList.remove('pzr-open');
		panel.hidden = true;
		panel.style.height = '';
		panel.style.transform = '';
		unlockBody();
	...
	function maybeShowLead() {
		...
		if (lead) {
			lead.hidden = false;
			root.classList.add('pzr-lead-active');
			var nameField = lead.querySelector('.pzr-lead-name');
	...
	if (lead) {
		var startBtn = lead.querySelector('.pzr-lead-start');
		var skipBtn = lead.querySelector('.pzr-lead-skip');
		var errBox = lead.querySelector('.pzr-lead-error');

		startBtn.addEventListener('click', function () {
			var name = (lead.querySelector('.pzr-lead-name').value || '').trim();
			var phone = (lead.querySelector('.pzr-lead-phone').value || '').trim();
	...
	function appendMessage(text, who) {
		...
		el.className = 'pzr-msg ' + (who === 'user' ? 'pzr-msg-user' : 'pzr-msg-bot');
	...
	function showTyping() {
		var el = document.createElement('div');
		el.className = 'pzr-msg pzr-msg-bot pzr-typing';
	...
	function renderCtaCard(card) {
		...
		wrap.className = 'pzr-cta-card';

		if (root.dataset.chBooking === '1' && bookingUrl) {
			var head = document.createElement('div');
			head.className = 'pzr-cta-head';
			head.innerHTML = '<div class="pzr-cta-title"></div><div class="pzr-cta-text"></div>';
			head.querySelector('.pzr-cta-title').textContent = cfg.strings.ctaTitle;
			head.querySelector('.pzr-cta-text').textContent = cfg.strings.ctaText;
			wrap.appendChild(head);
			wrap.appendChild(channelBtn(cfg.strings.book, bookingUrl, 'booking', '📅', true));
		}

		var row = document.createElement('div');
		row.className = 'pzr-channel-row';
	...
	function channelBtn(label, href, type, emoji, primary) {
		...
		a.className = 'pzr-cta-btn pzr-cta-' + type + (primary ? ' pzr-cta-primary' : '');
		a.href = href;
		a.target = '_blank';
		a.rel = 'noopener';
		a.innerHTML = '<span class="pzr-cta-emoji" aria-hidden="true"></span><span class="pzr-cta-label"></span>';
		a.querySelector('.pzr-cta-emoji').textContent = emoji;
		a.querySelector('.pzr-cta-label').textContent = label;
	...
	function ajaxFallback(text) {
		...
		var p = payload(text);
		body.append('action', 'pzr_chat_message');
	...
	function trackEvent(type) {
		...
		fetch(cfg.eventUrl, {
			...
			var body = new URLSearchParams();
			body.append('action', 'pzr_track_event');
			body.append('nonce', cfg.ajaxNonce);
	...
	if (quickWrap) {
		quickWrap.addEventListener('click', function (e) {
			var btn = e.target.closest('.pzr-quick-reply');
	...
	(function initTeaser() {
		...
		function reveal() {
			...
			teaser.hidden = false;
			root.classList.add('pzr-teaser-on');
		...
		function dismiss(persist) {
			clearTimeout(timer);
			teaser.hidden = true;
			root.classList.remove('pzr-teaser-on');
			if (persist) { store('pzr_teaser_dismissed', '1'); }
		}

		var closeBubble = teaser.querySelector('.pzr-teaser-close');
		if (closeBubble) {
