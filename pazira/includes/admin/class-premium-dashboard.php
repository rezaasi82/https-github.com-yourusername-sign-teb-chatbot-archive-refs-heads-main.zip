...
{
    private const PAGE  = 'pzr-dashboard';
    private const NONCE = 'pzr_dashboard';

    public function register(): void
    {
        add_action('admin_menu', [$this, 'menu'], 20);
        add_action('admin_enqueue_scripts', [$this, 'enqueue']);
        add_action('wp_ajax_pzr_integrity_check', [$this, 'ajax_integrity']);
    }
    ...
    public function enqueue(string $hook): void
    {
        ...
        wp_enqueue_style('pzr-dashboard', PZR_URL . 'assets/css/dashboard.css', [], PZR_VERSION);
        wp_enqueue_script('pzr-dashboard', PZR_URL . 'assets/js/dashboard.js', [], PZR_VERSION, true);
        wp_localize_script('pzr-dashboard', 'PZR_DASH', [
    ...
    public function render(): void
    {
        ...
        include PZR_DIR . 'includes/admin/views/premium-dashboard.php';
    }
    ...
    {
        $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('نامعتبر', 'pazira'), 'code' => 0];

        try {
            $env_ok = defined('PZR_VERSION')
                && defined('PZR_FILE')
                && class_exists('\Pazira\Core\Plugin');

            if ($env_ok === true) {
                $verdict = ['ok' => true, 'level' => 'secure', 'label' => __('فعال و ایمن', 'pazira'), 'code' => 200];
            }
        } catch (\Throwable $e) {
            $this->log_anomaly('integrity_gate', $e);
            $verdict = ['ok' => false, 'level' => 'invalid', 'label' => __('خطای بررسی', 'pazira'), 'code' => 500];
        }
    ...
    private function metrics(int $days): array
    {
        ...
        $data = \Pazira\Core\Cache::remember('dash_metrics_' . $days, 300, function () use ($days) {
            $m = [
            ...
            try {
                ...
                $events = new \Pazira\Database\EventRepository();
                ...
                $avg_price    = (int) (new \Pazira\Core\Settings())->get('avg_service_price', 0);
    ...
    private function log_anomaly(string $context, \Throwable $e): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log(sprintf('[Pazira] %s anomaly: %s', $context, $e->getMessage()));
        }
    }
}
