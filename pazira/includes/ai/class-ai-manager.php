...
class AiManager
{
    ...
    private \Pazira\Safety\MedicalSafetyFilter $safety;
    private \Pazira\Ai\SystemPromptBuilder $prompt;
    private \Pazira\Ai\LanguageDetector $language;
    private \Pazira\Ai\CtaDetector $cta;
    private \Pazira\Ai\LeadScorer $scorer;
    private \Pazira\Ai\SummaryBuilder $summary;
    private \Pazira\Ai\ProviderFactory $providers;
    private \Pazira\Database\ConversationRepository $conversations;
    private \Pazira\Database\MessageRepository $messages;

    public function __construct()
    {
        $this->settings      = new \Pazira\Core\Settings();
        $this->safety        = new \Pazira\Safety\MedicalSafetyFilter($this->settings);
        $this->prompt        = new \Pazira\Ai\SystemPromptBuilder($this->settings);
        $this->language      = new \Pazira\Ai\LanguageDetector();
        $this->cta           = new \Pazira\Ai\CtaDetector();
        $this->scorer        = new \Pazira\Ai\LeadScorer();
        $this->summary       = new \Pazira\Ai\SummaryBuilder($this->settings);
        $this->providers     = new \Pazira\Ai\ProviderFactory($this->settings);
        $this->conversations = new \Pazira\Database\ConversationRepository();
        $this->messages      = new \Pazira\Database\MessageRepository();
    }
    ...
    public function handle(array $req): array
    {
        if (! $this->settings->is_enabled()) {
            return ['ok' => false, 'code' => 'disabled', 'error' => __('چت‌بات غیرفعال است.', 'pazira')];
        }

        $message = trim((string) $req['message']);
        if ($message === '') {
            return ['ok' => false, 'code' => 'empty', 'error' => __('پیام خالی است.', 'pazira')];
        }
        ...
        $limiter = new \Pazira\Ratelimit\RateLimiter((int) $this->settings->get('rate_limit_per_min', 8));
        if (! $limiter->allow($req['ip'] . '|' . $req['session_id'])) {
            return ['ok' => false, 'code' => 'rate_limited', 'error' => __('لطفاً کمی صبر کنید و دوباره تلاش کنید.', 'pazira')];
        }
        ...
        $context = [
            ...
            'max_tokens'  => 1200,
            'temperature' => (float) apply_filters('pzr_temperature', 0.8),
        ];
        ...
        if ($cta !== '') {
            ...
            do_action('pzr_lead_detected', $conversation_id, $cta);
        }
        ...
        do_action('pzr_message_handled', $conversation_id, $cta, $score['level']);
