...
class LeadScorer
{
    ...
    public function score(array $signals): array
    {
        ...
        if ($points >= 5) {
            return ['level' => 'hot', 'label' => __('لید داغ', 'pazira'), 'emoji' => '🟢', 'probability' => min(95, 78 + $points)];
        }
        if ($points >= 2) {
            return ['level' => 'warm', 'label' => __('لید متوسط', 'pazira'), 'emoji' => '🟡', 'probability' => min(74, 45 + $points * 4)];
        }
        return ['level' => 'cold', 'label' => __('لید سرد', 'pazira'), 'emoji' => '⚪', 'probability' => max(5, 12 + $points * 4)];
    }
