...
{
    private const NONCE = 'pzr_export';

    public function register(): void
    {
        add_action('wp_ajax_pzr_export_pdf', [$this, 'export_pdf']);
        add_action('wp_ajax_pzr_download_pdf', [$this, 'download_pdf']);
        add_action('wp_ajax_pzr_export_webhook', [$this, 'export_webhook']);
        add_action('wp_ajax_pzr_export_gsheet', [$this, 'export_gsheet']);
        add_action('wp_ajax_pzr_test_webhook', [$this, 'test_webhook']);
        add_action('wp_ajax_pzr_test_gsheet', [$this, 'test_gsheet']);
        add_action('wp_ajax_pzr_test_sms', [$this, 'test_sms']);
        add_action('wp_ajax_pzr_sms_diag', [$this, 'sms_diag']);
        add_action('wp_ajax_pzr_send_sms', [$this, 'send_sms']);
        add_action('wp_ajax_pzr_export_bulk', [$this, 'bulk']);
    }
    ...
    public function sms_diag(): void
    {
        ...
        $this->guard();
        $diag = (new \Pazira\Notifications\SmsManager())->diagnose();
    ...
    public function test_sms(): void
    {
        \Pazira\Core\JsonGuard::arm();
        $this->guard();
        $to = \Pazira\Core\Input::post_text('to');
        if ($to === '') {
            wp_send_json(['ok' => false, 'error' => __('شماره مقصد را وارد کنید.', 'pazira')], 400);
        }

        $sms = new \Pazira\Notifications\SmsManager();
        if ($sms->template_code('welcome') !== '') {
            $sample = (object) [
                'patient_name'  => __('کاربر آزمایشی', 'pazira'),
                'patient_phone' => $to,
                'lead_score'    => 'warm',
                'lead_status'   => 'new',
                'summary'       => '',
        ...
        $text   = sprintf(__('پیام آزمایشی از %s (Pazira).', 'pazira'), $clinic);
    ...
    public function send_sms(): void
    {
        ...
        $to       = \Pazira\Core\Input::post_text('to');
        $tpl_key  = \Pazira\Core\Input::post_key('template', 'referral');
        if ($lead_id <= 0 || $to === '') {
            wp_send_json(['ok' => false, 'error' => __('لید یا شماره مقصد نامعتبر است.', 'pazira')], 400);
        }
    ...
    private function guard(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Pazira\Security\Security::note_failure('export_ajax');
    ...
    public function export_pdf(): void
    {
        ...
        $this->guard();
        $lead_id = \Pazira\Core\Input::post_int('lead_id');
        wp_send_json((new \Pazira\Export\ExportManager())->export_pdf($lead_id));
    }
    ...
    public function export_webhook(): void
    {
        ...
        $this->guard();
        $lead_id = \Pazira\Core\Input::post_int('lead_id');
        wp_send_json((new \Pazira\Export\ExportManager())->export_webhook($lead_id, 'manual'));
    }
    ...
    public function export_gsheet(): void
    {
        ...
        $this->guard();
        $lead_id = \Pazira\Core\Input::post_int('lead_id');
        wp_send_json((new \Pazira\Export\ExportManager())->export_google_sheet($lead_id));
    }
    ...
    public function test_webhook(): void
    {
        \Pazira\Core\JsonGuard::arm();
        $this->guard();
        wp_send_json((new \Pazira\Export\WebhookManager())->test());
    }
    ...
    public function test_gsheet(): void
    {
        \Pazira\Core\JsonGuard::arm();
        $this->guard();
        wp_send_json((new \Pazira\Export\GoogleSheets())->test());
    }
    ...
    public function bulk(): void
    {
        ...
        $op  = \Pazira\Core\Input::post_key('op');
        $ids = array_map('absint', (array) (\Pazira\Core\Input::post_fields()['ids'] ?? []));
        ...
        $queue = new \Pazira\Jobs\JobQueue();
        foreach ($ids as $id) {
    ...
    {
        if (\Pazira\Security\Security::is_locked()) {
            wp_die(esc_html__('دسترسی موقتاً مسدود شده است.', 'pazira'), '', ['response' => 429]);
        }
        if (! current_user_can('manage_options') || ! check_admin_referer(self::NONCE, 'nonce')) {
            \Pazira\Security\Security::note_failure('pdf_download');
            wp_die(esc_html__('دسترسی غیرمجاز.', 'pazira'), '', ['response' => 403]);
        }
        $lead_id = \Pazira\Core\Input::get_int('lead_id');

        $manager = new \Pazira\Export\ExportManager();
        $conv    = (new \Pazira\Database\ConversationRepository())->get($lead_id);
        ...
        if ($path === '' || ! is_readable($path)) {
            wp_die(esc_html__('فایل یافت نشد.', 'pazira'), '', ['response' => 404]);
        }

        $is_pdf   = str_ends_with($path, '.pdf');
        $filename = 'pazira-lead-' . $lead_id . ($is_pdf ? '.pdf' : '.html');
        nocache_headers();
     ...
     * inside the plugin's pazira-pdf folder.
     */
    private function url_to_path(string $url): string
    {
        ...
        if (strpos($url, $uploads['baseurl'] . '/pazira-pdf/') !== 0) {
            return '';
        }
        $path = $uploads['basedir'] . substr($url, strlen($uploads['baseurl']));
        $real = realpath($path);
        $base = realpath($uploads['basedir'] . '/pazira-pdf');
    ...
    private function delete_files(int $lead_id): bool
    {
        ...
        (new \Pazira\Database\ConversationRepository())->set_pdf_url($lead_id, '');
        return true;
    }
}
