<?php
/**
 * The plugin side of the Pazira Cloud platform (Level 2).
 *
...
class CloudClient
{
    public const OPTION_SECRET   = 'pzr_cloud_secret_enc';
    public const OPTION_REGISTERED = 'pzr_cloud_registered';
    private const CRON_HEARTBEAT = 'pzr_cloud_heartbeat';
    private const CRON_INSTALL   = 'pzr_cloud_install';
    ...
    public static function save_secret(string $plain): void
    {
        ...
        update_option(self::OPTION_SECRET, \Pazira\Core\Encryption::encrypt($plain), false);
    }
    ...
    private function telemetry(): array
    {
        return [
            ...
            'plugin_version' => defined('PZR_VERSION') ? PZR_VERSION : '',
    ...
    {
        global $wpdb;
        $conv = \Pazira\Database\Schema::conversations_table();
        $msg  = \Pazira\Database\Schema::messages_table();
        return [
            ...
            'active_24h' => (new \Pazira\Database\ConversationRepository())->active_count(24),
        ];
    }
    ...
    private function send(string $type, array $payload): bool
    {
        ...
        $headers = [
            ...
            'X-Pazira-Type'  => $type,
            'X-Pazira-Time'  => (string) time(),
        ];
        if ($secret !== '') {
            $headers['X-Pazira-Sign'] = 'sha256=' . hash_hmac('sha256', $body, $secret);
        }
    ...
    private function log(\Throwable $e): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Pazira] cloud client: ' . $e->getMessage());
        }
    }
}
