...
class Activator
{
    ...
    {
        \Pazira\Database\Schema::install();
        self::seed_default_settings();
        if (! wp_next_scheduled(\Pazira\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Pazira\Jobs\Rollup::CRON);
        }
        flush_rewrite_rules();
    }
    ...
    {
        if (get_option('pzr_db_version') !== \Pazira\Database\Schema::DB_VERSION) {
            \Pazira\Database\Schema::install();
        }
        if (! wp_next_scheduled(\Pazira\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Pazira\Jobs\Rollup::CRON);
    ...
    public static function default_settings(): array
    {
        return [
            ...
            'bot_name'           => __('دستیار هوشمند', 'pazira'),
            'avatar_url'         => '',
            ...
            // --- Messaging ---
            'welcome_message'    => __('سلام! 👋 چطور می‌تونم کمکتون کنم؟', 'pazira'),
            'quick_replies'      => "هزینه ویزیت\nآدرس کلینیک\nرزرو نوبت",
            'business_hours'     => '',
            'offhours_message'   => __('در حال حاضر خارج از ساعت کاری هستیم، اما می‌تونم به سوالاتتون پاسخ بدم.', 'pazira'),
    ...
    private static function seed_default_settings(): void
    {
        ...
        update_option(\Pazira\Core\Settings::OPTION, array_merge(self::default_settings(), $existing));
    }
}
