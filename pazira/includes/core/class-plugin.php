...
class Plugin
{
    ...
    private static function licensed(): bool
    {
        ...
        $file = PZR_DIR . 'includes/' . $class . '.php';
    ...
    public function boot(): void
    {
        ...
        load_plugin_textdomain('pazira', false, dirname(PZR_BASENAME) . '/languages');

        // Lightweight schema check in case the plugin was updated in place.
        \Pazira\Core\Activator::maybe_upgrade();

        // REST transport (preferred).
        (new \Pazira\Rest\ChatController())->register_routes();
        (new \Pazira\Rest\ExportController())->register_routes();

        // admin-ajax fallback for hosts that block the REST API.
        (new \Pazira\Ajax\ChatAjaxHandler())->register();

        // Export module: cron retries + automatic lead-event triggers.
        (new \Pazira\Export\ExportManager())->register();

        // Level 2 telemetry spine (opt-in; no-op when disabled).
        (new \Pazira\Cloud\CloudClient())->register();

        // Background jobs + daily analytics rollup (cron workers).
        (new \Pazira\Jobs\Rollup())->register();
        (new \Pazira\Jobs\JobQueue())->register();

        // Security audit trail (event listeners + admin viewer).
        (new \Pazira\Security\AuditLog())->register();

        // Inline chat via [pazira_chat] — registered on both front-end and
        // admin so the shortcode resolves in the block editor preview too.
        (new \Pazira\Frontend\ChatShortcode())->register();

        // Instant lead alerts to the clinic's Bale / Telegram (fires on the
        // front-end lead-detection hook + the admin test endpoint).
        (new \Pazira\Notifications\MessengerNotifier())->register();
        ...
        if (is_admin()) {
            ...
            (new \Pazira\Admin\AdminMenu())->register();
            (new \Pazira\Admin\ChatNotifier())->register();
            (new \Pazira\Admin\PremiumDashboard())->register();
            (new \Pazira\Admin\CrmBoard())->register();
            (new \Pazira\Admin\BranchesPage())->register();
            (new \Pazira\Seo\SeoPage())->register();
            (new \Pazira\Ajax\ExportAjaxHandler())->register();
            (new \Pazira\Crm\LeadCrm())->register();
        } else {
            (new \Pazira\Frontend\Widget())->register();
        }
    }
}
