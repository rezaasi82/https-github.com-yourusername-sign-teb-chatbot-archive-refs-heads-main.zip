<?php
/**
 * Typed accessor over the pzr_settings option array.
 *
...
class Settings
{
    public const OPTION = 'pzr_settings';

    /** @var array<string,string> provider => option name holding the encrypted key */
    private const KEY_OPTIONS = [
        'anthropic' => 'pzr_api_key_anthropic_enc',
        'openai'    => 'pzr_api_key_openai_enc',
        'gapgpt'    => 'pzr_api_key_gapgpt_enc',
    ];
    ...
    {
        return (new \Pazira\Ai\ProviderFactory($this))->model_for($this->active_provider());
    }
    ...
    public function get_api_key(?string $provider = null): string
    {
        ...
        return is_string($encrypted) ? \Pazira\Core\Encryption::decrypt($encrypted) : '';
    }
