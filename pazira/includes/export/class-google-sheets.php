...
class GoogleSheets
{
    public const OPTION_SECRET = 'pzr_gsheet_secret_enc';
    ...
    public function send(array $row): array
    {
        ...
        $secret  = $this->secret();
        if ($secret !== '') {
            $headers['X-Pazira-Secret'] = $secret;
        }
    ...
    {
        return $this->send(['TEST', 'Pazira', '-', 'test', '-', current_time('mysql'), 'connection test', '-', 'test']);
    }
