 ...
 *
 * A lead is a conversation (pzr_conversations) plus its messages
 * (pzr_messages). This is the single representation consumed by every export
...
class LeadPayload
{
    ...
    public function __construct(?\Pazira\Database\ConversationRepository $conversations = null, ?\Pazira\Database\MessageRepository $messages = null)
    {
        $this->conversations = $conversations ?? new \Pazira\Database\ConversationRepository();
        $this->messages      = $messages ?? new \Pazira\Database\MessageRepository();
    }
