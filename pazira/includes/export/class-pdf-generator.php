 ...
 * under uploads/pazira-pdf/YYYY/MM/ with an unguessable name.
 *
 * @package Pazira
 */
...
class PdfGenerator
{
    ...
    public function __construct(?\Pazira\Core\Settings $settings = null, ?\Pazira\Export\LeadPayload $payloads = null)
    {
    ...
    private function target_dir(): ?array
    {
        ...
        $sub  = '/pazira-pdf/' . gmdate('Y') . '/' . gmdate('m');
        $path = $uploads['basedir'] . $sub;
        if (! wp_mkdir_p($path)) {
            return null;
        }
        // Prevent directory listing.
        if (! file_exists($uploads['basedir'] . '/pazira-pdf/index.php')) {
            file_put_contents($uploads['basedir'] . '/pazira-pdf/index.php', "<?php\n// Silence is golden.\n");
        }
    ...
    public function render_html(array $payload): string
    {
        ...
        foreach ($payload['messages'] as $m) {
            $is_user = $m['role'] === 'user';
            $who     = $is_user ? 'بیمار' : 'دستیار Pazira';
            $side    = $is_user ? 'pzr-p' : 'pzr-a';
...
<style>
    * { box-sizing: border-box; }
    ...
    .pzr-p { background: #eef2fb; border: 1px solid #dbe2f2; margin-inline-start: auto; }
    .pzr-a { background: #fff; border: 1px solid #e6e9f0; }
...
<div class="report">
    ...
    <div class="foot">
        <span>Pazira · <?php echo esc_html($exported); ?></span>
