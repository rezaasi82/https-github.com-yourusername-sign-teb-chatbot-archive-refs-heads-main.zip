...
{
    public const OPTION_SECRET = 'pzr_webhook_secret_enc';
    public const MAX_ATTEMPTS  = 3;

    private \Pazira\Core\Settings $settings;
    private \Pazira\Export\ExportLogger $logger;
    private \Pazira\Export\LeadPayload $payloads;

    public function __construct(?\Pazira\Core\Settings $settings = null, ?\Pazira\Export\ExportLogger $logger = null, ?\Pazira\Export\LeadPayload $payloads = null)
    {
        $this->settings = $settings ?? new \Pazira\Core\Settings();
        $this->logger   = $logger ?? new \Pazira\Export\ExportLogger();
        $this->payloads = $payloads ?? new \Pazira\Export\LeadPayload();
    }
    ...
    public function dispatch(int $lead_id, string $event, string $pdf_url = '', int $attempt = 1): array
    {
        ...
        if ($should_retry) {
            wp_schedule_single_event(time() + 300 * $attempt, 'pzr_webhook_retry', [$lead_id, $event, $pdf_url, $attempt + 1]);
        }
    ...
    public function send(array $payload, string $event = 'manual'): array
    {
        ...
        $headers = [
            ...
            'X-Pazira-Event'  => $event,
        ];
        if ($secret !== '') {
            $headers['X-Pazira-Signature'] = 'sha256=' . hash_hmac('sha256', (string) $body, $secret);
        }
