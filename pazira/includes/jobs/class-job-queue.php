 ...
 * drained in small batches on the pzr_process_jobs cron, so an admin request
...
class JobQueue
{
    public const CRON  = 'pzr_process_jobs';
    ...
    public function enqueue(string $type, array $payload): int
    {
        ...
        $wpdb->insert(
            \Pazira\Database\Schema::jobs_table(),
            [
    ...
    {
        global $wpdb;
        $table = \Pazira\Database\Schema::jobs_table();
        ...
        foreach ($jobs as $job) {
            ...
            try {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('[Pazira] job #' . $job->id . ' error: ' . $e->getMessage());
                }
            }
