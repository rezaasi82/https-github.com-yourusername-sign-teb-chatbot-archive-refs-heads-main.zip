 ...
 *
 * Runs on the pzr_daily_rollup cron and writes one aggregated row per
 * (day, metric) into pzr_analytics, so long-range dashboards read a few
 * indexed rows instead of scanning the raw tables.
 *
 * @package Pazira
 */

namespace Pazira\Jobs;
...
class Rollup
{
    public const CRON = 'pzr_daily_rollup';
    ...
    private function day_metrics(string $day): array
    {
        ...
        $events = \Pazira\Database\Schema::events_table();
        $start  = $day . ' 00:00:00';
        $end    = $day . ' 23:59:59';
