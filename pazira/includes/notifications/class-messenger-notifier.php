...
class MessengerNotifier
{
    ...
    private const CHANNELS = [
        'bale'     => ['https://tapi.bale.ai', 'pzr_msgr_bale_token_enc', 'بله'],
        'telegram' => ['https://api.telegram.org', 'pzr_msgr_tg_token_enc', 'Telegram'],
    ];
    ...
    {
        add_action('pzr_lead_detected', [$this, 'notify_new_lead'], 10, 2);
        add_action('wp_ajax_pzr_test_messenger', [$this, 'ajax_test']);
    }
    ...
    public function send(string $channel, string $text): array
    {
        ...
        if ($token === '' || $chat === '') {
            return ['ok' => false, 'error' => __('توکن ربات یا شناسه چت تنظیم نشده است.', 'pazira')];
        }
        ...
        return ['ok' => false, 'code' => $code, 'error' => $desc !== '' ? $desc : sprintf(__('پاسخ سرویس: HTTP %d', 'pazira'), $code)];
    }
    ...
    public function notify_new_lead(int $conversation_id, string $cta = ''): void
    {
        ...
        $c = (new \Pazira\Database\ConversationRepository())->get($conversation_id);
        if (! $c) {
            return;
        }
        ...
        foreach ($enabled as $channel) {
            ...
            \Pazira\Security\AuditLog::record('messenger_notify', ['object' => $channel, 'severity' => ! empty($r['ok']) ? 'info' : 'warning']);
        }
    }
    ...
    private function lead_text(object $c, string $cta): string
    {
        ...
        $clinic = $vars['clinic'];
        $lines  = [
            '🔔 ' . sprintf(__('لید جدید در %s', 'pazira'), $clinic),
            sprintf(__('نام: %s', 'pazira'), $vars['name']),
            sprintf(__('موبایل: %s', 'pazira'), $vars['phone'] !== '' ? $vars['phone'] : '—'),
            sprintf(__('امتیاز: %s', 'pazira'), $vars['score']),
        ];
        if ($cta !== '') {
            $lines[] = sprintf(__('اقدام: %s', 'pazira'), $cta);
        }
        if ($vars['summary'] !== '') {
            $lines[] = "\n" . $vars['summary'];
        }
    ...
    public function ajax_test(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Pazira\Security\Security::note_failure('messenger_test');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        $channel = \Pazira\Core\Input::post_key('channel');
        if (! isset(self::CHANNELS[$channel])) {
            wp_send_json(['ok' => false, 'error' => 'bad_channel'], 400);
        }
        $clinic = (string) $this->settings->get('clinic_name', get_bloginfo('name'));
        wp_send_json($this->send($channel, sprintf(__('پیام آزمایشی %s از Pazira ✅', 'pazira'), $clinic)));
    }
}
