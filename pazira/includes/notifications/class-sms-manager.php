...
namespace Pazira\Notifications;
...
class SmsManager
{
    public const OPTION_KEY    = 'pzr_sms_key_enc';
    public const OPTION_SECRET = 'pzr_sms_secret_enc';

    /** id => provider class. Add a panel by adding one line + one class. */
    private const PROVIDERS = [
        'kavenegar'   => '\Pazira\Notifications\Providers\SmsKavenegar',
        'melipayamak' => '\Pazira\Notifications\Providers\SmsMelipayamak',
        'smsir'       => '\Pazira\Notifications\Providers\SmsSmsir',
        'ghasedak'    => '\Pazira\Notifications\Providers\SmsGhasedak',
        'custom'      => '\Pazira\Notifications\Providers\SmsCustom',
    ];

    private \Pazira\Core\Settings $settings;

    public function __construct(?\Pazira\Core\Settings $settings = null)
    {
        $this->settings = $settings ?? new \Pazira\Core\Settings();
    }
    ...
    public function active(): ?\Pazira\Notifications\SmsProviderInterface
    {
    ...
    public function send(string $to, string $text): array
    {
        $text = trim($text);
        if ($text === '') {
            return ['ok' => false, 'error' => __('متن پیام خالی است.', 'pazira')];
        }
        return $this->dispatch(static fn(\Pazira\Notifications\SmsProviderInterface $p) => $p->send($to, $text));
    }
    ...
    public function send_lead(string $to, string $template_key, array $vars): array
    {
        ...
        return $this->dispatch(function (\Pazira\Notifications\SmsProviderInterface $p) use ($to, $tpl_text, $code, $vars, $order) {
    ...
    private function dispatch(callable $call): array
    {
        if (! $this->is_configured()) {
            return ['ok' => false, 'error' => __('پنل پیامک پیکربندی نشده است.', 'pazira')];
        }
        $provider = $this->active();
        if ($provider === null) {
            return ['ok' => false, 'error' => __('سرویس پیامک نامعتبر است.', 'pazira')];
        }
        $result = $call($provider);
        \Pazira\Security\AuditLog::record('sms_send', ['object' => $provider->id(), 'severity' => ! empty($result['ok']) ? 'info' : 'warning']);
        return $result;
    }
    ...
    {
        $lines   = [];
        $lines[] = sprintf(__('سرویس انتخاب‌شده: %s', 'pazira'), $this->providers()[$this->active_id()] ?? $this->active_id());
        $lines[] = sprintf(__('فعال‌سازی: %s', 'pazira'), $this->is_enabled() ? '✓' : __('✗ (تیک «فعال‌سازی» را بزنید)', 'pazira'));
        $lines[] = sprintf(__('APIKey ذخیره شده: %s', 'pazira'), self::key() !== '' ? '✓' : '✗');
        $lines[] = sprintf(__('رمز عبور ذخیره شده: %s', 'pazira'), self::secret() !== '' ? __('✓ (حالت نام‌کاربری/رمز)', 'pazira') : __('— (حالت توکن)', 'pazira'));
        $lines[] = sprintf(__('شماره فرستنده: %s', 'pazira'), trim((string) $this->settings->get('sms_sender', '')) !== '' ? $this->settings->get('sms_sender') : __('خالی (برای خط اشتراکی درست است)', 'pazira'));

        $codes = array_filter(array_map([$this, 'template_code'], array_keys(self::default_templates())));
        $lines[] = sprintf(__('کد الگو تنظیم‌شده: %d قالب', 'pazira'), count($codes));
        ...
        $lines[] = __('این سرویس بررسی زنده ندارد؛ با «ارسال پیامک تست» امتحان کنید.', 'pazira');
    ...
    public static function key(): string
    {
        ...
        return is_string($enc) && $enc !== '' ? \Pazira\Core\Encryption::decrypt($enc) : '';
    }
    ...
    private static function store(string $option, string $plain): void
    {
        ...
        update_option($option, \Pazira\Core\Encryption::encrypt($plain), false);
    }
    ...
    public static function default_templates(): array
    {
        ...
        return [
            'welcome'  => [
                'label' => __('خوش‌آمد به لید', 'pazira'),
                'text'  => __('{0} عزیز، از تماس شما با {1} سپاسگزاریم. کارشناسان ما به‌زودی برای هماهنگی با شما تماس می‌گیرند. 🌿 {optout}', 'pazira'),
                'vars'  => ['name', 'clinic'],
            ],
            'referral' => [
                'label' => __('ارجاع لید به همکار', 'pazira'),
                'text'  => __('لید جدید در سایت : نام {0} -شماره {1} - امتیاز: {2}. لطفاً پیگیری کنید. {optout}', 'pazira'),
                'vars'  => ['name', 'phone', 'score'],
            ],
            'reminder' => [
                'label' => __('یادآوری پیگیری', 'pazira'),
                'text'  => __('{0} عزیز، جهت تکمیل مشاوره و رزرو نوبت با {1} در ارتباط باشید. منتظر شما هستیم. {optout}', 'pazira'),
                'vars'  => ['name', 'clinic'],
            ],
            'custom'   => [
                'label' => __('پیام سفارشی', 'pazira'),
                'text'  => __('{0} عزیز، {1} در خدمت شماست. {optout}', 'pazira'),
                'vars'  => ['name', 'clinic'],
            ],
        ];
    }
    ...
    {
        $scores = ['hot' => __('داغ', 'pazira'), 'warm' => __('متوسط', 'pazira'), 'cold' => __('سرد', 'pazira')];
        return [
            'name'    => trim((string) ($c->patient_name ?? '')) ?: __('کاربر', 'pazira'),
            'phone'   => trim((string) ($c->patient_phone ?? '')),
            'score'   => $scores[$c->lead_score ?? ''] ?? '—',
            'status'  => \Pazira\Crm\LeadCrm::label((string) ($c->lead_status ?? 'new')),
    ...
    public static function messenger_links(string $phone, string $text): array
    {
        ...
        return [
            'sms'      => ['label' => __('پیامک دستگاه', 'pazira'), 'url' => 'sms:' . $digits . '?&body=' . $enc],
