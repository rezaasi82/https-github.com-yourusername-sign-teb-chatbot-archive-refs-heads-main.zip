...
abstract class SmsProviderBase implements \Pazira\Notifications\SmsProviderInterface
{
    protected \Pazira\Core\Settings $settings;
    ...
    {
        return ['ok' => false, 'error' => __('این سرویس از ارسال الگو پشتیبانی نمی‌کند.', 'pazira')];
    }
    ...
    {
        return \Pazira\Notifications\SmsManager::key();
    }

    /** Optional second credential (e.g. MeliPayamak password). */
    protected function api_secret(): string
    {
        return \Pazira\Notifications\SmsManager::secret();
    }
