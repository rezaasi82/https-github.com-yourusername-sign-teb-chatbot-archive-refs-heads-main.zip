...
class SmsCustom extends \Pazira\Notifications\SmsProviderBase
{
    ...
    {
        return __('سفارشی / خارجی (Custom HTTP)', 'pazira');
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($url === '') {
            return ['ok' => false, 'error' => __('آدرس سرویس سفارشی تنظیم نشده است.', 'pazira')];
        }
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $r['error'] ?? sprintf(__('پاسخ سرویس: HTTP %d', 'pazira'), $r['code'])];
    }
