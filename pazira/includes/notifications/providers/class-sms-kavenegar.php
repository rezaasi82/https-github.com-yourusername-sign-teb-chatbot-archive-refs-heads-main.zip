...
class SmsKavenegar extends \Pazira\Notifications\SmsProviderBase
{
    ...
    public function check(): array
    {
        $key = $this->api_key();
        if ($key === '') {
            return ['ok' => false, 'detail' => __('هیچ APIKey ذخیره نشده است.', 'pazira')];
        }
        $r = $this->http('https://api.kavenegar.com/v1/' . rawurlencode($key) . '/account/info.json', ['method' => 'GET']);
        if (! empty($r['error'])) {
            return ['ok' => false, 'detail' => sprintf(__('سرور سایت به پنل دسترسی ندارد: %s', 'pazira'), $r['error'])];
        }
        $data = json_decode($r['body'], true);
        if ((int) ($data['return']['status'] ?? 0) === 200) {
            return ['ok' => true, 'detail' => sprintf(__('اتصال برقرار ✓ — اعتبار: %s', 'pazira'), (string) ($data['entries']['remaincredit'] ?? '?'))];
        }
        return ['ok' => false, 'detail' => sprintf(__('پنل اتصال را رد کرد: %s', 'pazira'), (string) ($data['return']['message'] ?? ('HTTP ' . $r['code'])))];
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($key === '' || $to === '') {
            return ['ok' => false, 'error' => __('کلید API یا شماره مقصد تنظیم نشده است.', 'pazira')];
        }
    ...
    public function send_pattern(string $to, string $code, array $params): array
    {
        ...
        $to  = $this->normalize($to);
        if ($key === '' || $to === '' || $code === '') {
            return ['ok' => false, 'error' => __('کلید API، شماره یا کد الگو تنظیم نشده است.', 'pazira')];
        }
