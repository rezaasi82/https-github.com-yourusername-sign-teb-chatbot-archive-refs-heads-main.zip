...
class SmsSmsir extends \Pazira\Notifications\SmsProviderBase
{
    ...
    public function check(): array
    {
        ...
        if ((int) ($data['status'] ?? 0) === 1) {
            return ['ok' => true, 'detail' => sprintf(__('اتصال برقرار ✓ — اعتبار: %s', 'pazira'), (string) ($data['data'] ?? '?'))];
        }
        return ['ok' => false, 'detail' => sprintf(__('پنل اتصال را رد کرد: %s', 'pazira'), (string) ($data['message'] ?? ('HTTP ' . $r['code'])))];
    }
