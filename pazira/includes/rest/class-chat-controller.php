...
class ChatController
{
    ...
    public function handle_message(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $result = (new \Pazira\Ai\AiManager())->handle([
            'session_id' => \Pazira\Rest\Sanitizer::session_id((string) $request->get_param('session_id')),
            'message'    => sanitize_textarea_field((string) $request->get_param('message')),
            'name'       => \Pazira\Rest\Sanitizer::name((string) $request->get_param('name')),
            'phone'      => \Pazira\Rest\Sanitizer::phone((string) $request->get_param('phone')),
