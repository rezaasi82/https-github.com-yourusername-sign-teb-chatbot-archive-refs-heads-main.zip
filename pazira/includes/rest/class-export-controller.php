...
{
    private const REST_NAMESPACE = 'pazira/v1';
    ...
    {
        \Pazira\Core\JsonGuard::arm();
        $payload = (new \Pazira\Export\LeadPayload())->build((int) $request['id']);
        if ($payload === null) {
    ...
    {
        \Pazira\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pazira\Export\ExportManager())->export_pdf((int) $request->get_param('lead_id')));
    }

    public function export_webhook(\WP_REST_Request $request): \WP_REST_Response
    {
        \Pazira\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pazira\Export\ExportManager())->export_webhook((int) $request->get_param('lead_id'), 'manual'));
    }

    public function export_google_sheet(\WP_REST_Request $request): \WP_REST_Response
    {
        \Pazira\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pazira\Export\ExportManager())->export_google_sheet((int) $request->get_param('lead_id')));
    }
}
