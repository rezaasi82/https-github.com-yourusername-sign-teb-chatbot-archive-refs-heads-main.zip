...
class SeoAnalyzer
{
    ...
    {
        return \Pazira\Core\Cache::remember('seo_q_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            global $wpdb;
            $messages = \Pazira\Database\Schema::messages_table();
    ...
    {
        return \Pazira\Core\Cache::remember('seo_kw_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            $counts = [];
    ...
    {
        return \Pazira\Core\Cache::remember('seo_topics_' . $days, 600, function () use ($days) {
            $services = array_map(
                static fn($s) => (string) $s['name'],
                (new \Pazira\Ai\SystemPromptBuilder(new \Pazira\Core\Settings()))->services()
            );
            if ($services === []) {
                return [];
            }
