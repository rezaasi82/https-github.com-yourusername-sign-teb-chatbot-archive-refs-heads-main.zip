...
class SettingsPage
{
    ...
    {
        $tab = \Pezhkam\Core\Input::get_key('tab', 'provider');
        return in_array($tab, self::TABS, true) ? $tab : 'provider';
    }

    public function handle_save(): void
    {
        if (! \Pezhkam\Core\Input::has_post('pzk_settings_submit')) {
            return;
        }
        if (! current_user_can('manage_options')) {
            return;
        }
        check_admin_referer('pzk_settings');
        ...
        $existing = get_option(\Pezhkam\Core\Settings::OPTION, []);
        ...
        if ($tab === 'provider') {
            ...
            if (isset($in['api_key_anthropic']) && trim((string) $in['api_key_anthropic']) !== '') {
                \Pezhkam\Core\Settings::save_api_key('anthropic', sanitize_text_field((string) $in['api_key_anthropic']));
            }
            if (isset($in['api_key_openai']) && trim((string) $in['api_key_openai']) !== '') {
                \Pezhkam\Core\Settings::save_api_key('openai', sanitize_text_field((string) $in['api_key_openai']));
            }
            if (isset($in['api_key_gapgpt']) && trim((string) $in['api_key_gapgpt']) !== '') {
                \Pezhkam\Core\Settings::save_api_key('gapgpt', sanitize_text_field((string) $in['api_key_gapgpt']));
            }
        } elseif ($tab === 'clinic') {
        ...
        update_option(\Pezhkam\Core\Settings::OPTION, array_merge($existing, $update));
        $this->finish($tab);
    }
    ...
    private function save_integrations(array $in): void
    {
        ...
        if (isset($in['sms_key']) && trim((string) $in['sms_key']) !== '') {
            \Pezhkam\Notifications\SmsManager::save_key((string) $in['sms_key']);
        }
        if (isset($in['sms_secret']) && trim((string) $in['sms_secret']) !== '') {
            \Pezhkam\Notifications\SmsManager::save_secret((string) $in['sms_secret']);
        }
        ...
        foreach (['bale', 'telegram'] as $ch) {
            if (isset($in['msgr_' . $ch . '_token']) && trim((string) $in['msgr_' . $ch . '_token']) !== '') {
                \Pezhkam\Notifications\MessengerNotifier::save_token($ch, (string) $in['msgr_' . $ch . '_token']);
            }
        }

        if (isset($in['webhook_secret']) && trim((string) $in['webhook_secret']) !== '') {
            \Pezhkam\Export\WebhookManager::save_secret((string) $in['webhook_secret']);
        }
        if (isset($in['gsheet_secret']) && trim((string) $in['gsheet_secret']) !== '') {
            \Pezhkam\Export\GoogleSheets::save_secret((string) $in['gsheet_secret']);
        }
        if (isset($in['cloud_secret']) && trim((string) $in['cloud_secret']) !== '') {
            \Pezhkam\Cloud\CloudClient::save_secret((string) $in['cloud_secret']);
        }
    }

    private function finish(string $tab): void
    {
        \Pezhkam\Security\AuditLog::record('settings_saved', ['object' => $tab, 'severity' => 'info']);
        add_settings_error('pzr', 'saved', __('تنظیمات ذخیره شد.', 'pezhkam'), 'updated');
        set_transient('settings_errors', get_settings_errors(), 30);
        wp_safe_redirect(admin_url('admin.php?page=pzk-chat&tab=' . $tab . '&updated=1'));
        exit;
    }
    ...
    public function render(): void
    {
        ...
        if ($float && $shortcode) {
            $display = __('ویجت شناور + شورت‌کد', 'pezhkam');
        } elseif ($float) {
            $display = __('فقط ویجت شناور', 'pezhkam');
        } elseif ($shortcode) {
            $display = __('فقط شورت‌کد', 'pezhkam');
        } else {
            $display = __('نمایش خاموش', 'pezhkam');
        }

        \Pezhkam\Admin\PageHeader::render(
            __('پژکام', 'pezhkam'),
            __('دستیار هوشمند جذب بیمار', 'pezhkam'),
            [
                [
                    'label' => $display,
                ...
                [
                    'label' => $has_key
                        ? __('کلید API تنظیم شده', 'pezhkam')
                        : __('کلید API تنظیم نشده', 'pezhkam'),
                    'state' => $has_key ? 'on' : 'off',
                ],
                ['label' => __('نسخه', 'pezhkam') . ' ' . PZK_VERSION],
            ]
        );
        ...
        if (in_array($tab, ['provider', 'clinic', 'appearance', 'integrations'], true)) {
            include PZK_DIR . 'includes/admin/views/settings.php';
        } elseif ($tab === 'conversations') {
            (new \Pezhkam\Admin\ConversationsPage())->render_inner();
        } elseif ($tab === 'stats') {
            (new \Pezhkam\Admin\StatsPage())->render_inner();
        }
    ...
    {
        $labels = [
            'provider'      => __('هوش مصنوعی', 'pezhkam'),
            'clinic'        => __('اطلاعات کلینیک', 'pezhkam'),
            'appearance'    => __('ظاهر ویجت', 'pezhkam'),
            'integrations'  => __('اتصال‌ها و خروجی', 'pezhkam'),
            'conversations' => __('لیدها و مکالمات', 'pezhkam'),
            'stats'         => __('آمار', 'pezhkam'),
        ];
        echo '<h2 class="nav-tab-wrapper">';
        foreach ($labels as $slug => $label) {
            $url    = admin_url('admin.php?page=pzk-chat&tab=' . $slug);
