...
class AiManager
{
    ...
    private \Pezhkam\Safety\MedicalSafetyFilter $safety;
    private \Pezhkam\Ai\SystemPromptBuilder $prompt;
    private \Pezhkam\Ai\LanguageDetector $language;
    private \Pezhkam\Ai\CtaDetector $cta;
    private \Pezhkam\Ai\LeadScorer $scorer;
    private \Pezhkam\Ai\SummaryBuilder $summary;
    private \Pezhkam\Ai\ProviderFactory $providers;
    private \Pezhkam\Database\ConversationRepository $conversations;
    private \Pezhkam\Database\MessageRepository $messages;

    public function __construct()
    {
        $this->settings      = new \Pezhkam\Core\Settings();
        $this->safety        = new \Pezhkam\Safety\MedicalSafetyFilter($this->settings);
        $this->prompt        = new \Pezhkam\Ai\SystemPromptBuilder($this->settings);
        $this->language      = new \Pezhkam\Ai\LanguageDetector();
        $this->cta           = new \Pezhkam\Ai\CtaDetector();
        $this->scorer        = new \Pezhkam\Ai\LeadScorer();
        $this->summary       = new \Pezhkam\Ai\SummaryBuilder($this->settings);
        $this->providers     = new \Pezhkam\Ai\ProviderFactory($this->settings);
        $this->conversations = new \Pezhkam\Database\ConversationRepository();
        $this->messages      = new \Pezhkam\Database\MessageRepository();
    }
    ...
    public function handle(array $req): array
    {
        if (! $this->settings->is_enabled()) {
            return ['ok' => false, 'code' => 'disabled', 'error' => __('چت‌بات غیرفعال است.', 'pezhkam')];
        }

        $message = trim((string) $req['message']);
        if ($message === '') {
            return ['ok' => false, 'code' => 'empty', 'error' => __('پیام خالی است.', 'pezhkam')];
        }
        ...
        $limiter = new \Pezhkam\Ratelimit\RateLimiter((int) $this->settings->get('rate_limit_per_min', 8));
        if (! $limiter->allow($req['ip'] . '|' . $req['session_id'])) {
            return ['ok' => false, 'code' => 'rate_limited', 'error' => __('لطفاً کمی صبر کنید و دوباره تلاش کنید.', 'pezhkam')];
        }
        ...
        $context = [
            ...
            'max_tokens'  => 1200,
            'temperature' => (float) apply_filters('pzk_temperature', 0.8),
        ];
        ...
        if ($cta !== '') {
            ...
            do_action('pzk_lead_detected', $conversation_id, $cta);
        }
        ...
        do_action('pzk_message_handled', $conversation_id, $cta, $score['level']);
