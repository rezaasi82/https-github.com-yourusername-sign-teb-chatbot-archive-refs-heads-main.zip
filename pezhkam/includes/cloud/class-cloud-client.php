<?php
/**
 * The plugin side of the Pezhkam Cloud platform (Level 2).
 *
...
class CloudClient
{
    public const OPTION_SECRET   = 'pzk_cloud_secret_enc';
    public const OPTION_REGISTERED = 'pzk_cloud_registered';
    private const CRON_HEARTBEAT = 'pzk_cloud_heartbeat';
    private const CRON_INSTALL   = 'pzk_cloud_install';
    ...
    private function telemetry(): array
    {
        return [
            ...
            'plugin_version' => defined('PZK_VERSION') ? PZK_VERSION : '',
    ...
    {
        global $wpdb;
        $conv = \Pezhkam\Database\Schema::conversations_table();
        $msg  = \Pezhkam\Database\Schema::messages_table();
        return [
            ...
            'active_24h' => (new \Pezhkam\Database\ConversationRepository())->active_count(24),
        ];
    }
    ...
    private function send(string $type, array $payload): bool
    {
        ...
        $headers = [
            ...
            'X-Pezhkam-Type'  => $type,
            'X-Pezhkam-Time'  => (string) time(),
        ];
        if ($secret !== '') {
            $headers['X-Pezhkam-Sign'] = 'sha256=' . hash_hmac('sha256', $body, $secret);
        }
    ...
    private function log(\Throwable $e): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Pezhkam] cloud client: ' . $e->getMessage());
        }
    }
}
