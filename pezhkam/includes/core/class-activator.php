...
class Activator
{
    ...
    {
        \Pezhkam\Database\Schema::install();
        self::seed_default_settings();
        if (! wp_next_scheduled(\Pezhkam\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Pezhkam\Jobs\Rollup::CRON);
        }
        flush_rewrite_rules();
    }
    ...
    {
        if (get_option('pzk_db_version') !== \Pezhkam\Database\Schema::DB_VERSION) {
            \Pezhkam\Database\Schema::install();
        }
        if (! wp_next_scheduled(\Pezhkam\Jobs\Rollup::CRON)) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', \Pezhkam\Jobs\Rollup::CRON);
    ...
    public static function default_settings(): array
    {
        return [
            ...
            'bot_name'           => __('دستیار هوشمند', 'pezhkam'),
            'avatar_url'         => '',
    ...
    private static function seed_default_settings(): void
    {
        ...
        update_option(\Pezhkam\Core\Settings::OPTION, array_merge(self::default_settings(), $existing));
    }
}
