...
class Plugin
{
    ...
    private static function licensed(): bool
    {
        ...
        $file = PZK_DIR . 'includes/' . $class . '.php';
    ...
    public function boot(): void
    {
        ...
        load_plugin_textdomain('pezhkam', false, dirname(PZK_BASENAME) . '/languages');

        // Lightweight schema check in case the plugin was updated in place.
        \Pezhkam\Core\Activator::maybe_upgrade();

        // REST transport (preferred).
        (new \Pezhkam\Rest\ChatController())->register_routes();
        (new \Pezhkam\Rest\ExportController())->register_routes();

        // admin-ajax fallback for hosts that block the REST API.
        (new \Pezhkam\Ajax\ChatAjaxHandler())->register();

        // Export module: cron retries + automatic lead-event triggers.
        (new \Pezhkam\Export\ExportManager())->register();

        // Level 2 telemetry spine (opt-in; no-op when disabled).
        (new \Pezhkam\Cloud\CloudClient())->register();

        // Background jobs + daily analytics rollup (cron workers).
        (new \Pezhkam\Jobs\Rollup())->register();
        (new \Pezhkam\Jobs\JobQueue())->register();

        // Security audit trail (event listeners + admin viewer).
        (new \Pezhkam\Security\AuditLog())->register();

        // Inline chat via [pezhkam_chat] — registered on both front-end and
        // admin so the shortcode resolves in the block editor preview too.
        (new \Pezhkam\Frontend\ChatShortcode())->register();

        // Instant lead alerts to the clinic's Bale / Telegram (fires on the
        // front-end lead-detection hook + the admin test endpoint).
        (new \Pezhkam\Notifications\MessengerNotifier())->register();
        ...
        if (is_admin()) {
            ...
            (new \Pezhkam\Admin\AdminMenu())->register();
            (new \Pezhkam\Admin\ChatNotifier())->register();
            (new \Pezhkam\Admin\PremiumDashboard())->register();
            (new \Pezhkam\Admin\CrmBoard())->register();
            (new \Pezhkam\Admin\BranchesPage())->register();
            (new \Pezhkam\Seo\SeoPage())->register();
            (new \Pezhkam\Ajax\ExportAjaxHandler())->register();
            (new \Pezhkam\Crm\LeadCrm())->register();
        } else {
            (new \Pezhkam\Frontend\Widget())->register();
        }
    }
}
