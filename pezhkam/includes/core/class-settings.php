<?php
/**
 * Typed accessor over the pzk_settings option array.
 *
...
class Settings
{
    public const OPTION = 'pzk_settings';

    /** @var array<string,string> provider => option name holding the encrypted key */
    private const KEY_OPTIONS = [
        'anthropic' => 'pzk_api_key_anthropic_enc',
        'openai'    => 'pzk_api_key_openai_enc',
        'gapgpt'    => 'pzk_api_key_gapgpt_enc',
    ];
    ...
    /** The [pezhkam_chat] shortcode / block embed. */
    ...
    {
        return (new \Pezhkam\Ai\ProviderFactory($this))->model_for($this->active_provider());
    }
    ...
    public function get_api_key(?string $provider = null): string
    {
        ...
        return is_string($encrypted) ? \Pezhkam\Core\Encryption::decrypt($encrypted) : '';
    }
