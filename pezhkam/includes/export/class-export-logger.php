...
class ExportLogger
{
    private \Pezhkam\Database\SyncLogRepository $repo;

    public function __construct(?\Pezhkam\Database\SyncLogRepository $repo = null)
    {
        $this->repo = $repo ?? new \Pezhkam\Database\SyncLogRepository();
    }
    ...
    public function finish(int $log_id, string $status, array $extra = []): void
    {
        ...
        if ($status === 'failed' && defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Pezhkam export] log #' . $log_id . ' failed: ' . ($update['response'] ?? ''));
        }
    }
}
