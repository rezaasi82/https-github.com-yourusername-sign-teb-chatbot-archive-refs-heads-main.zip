...
class GoogleSheets
{
    public const OPTION_SECRET = 'pzk_gsheet_secret_enc';
    ...
    public function send(array $row): array
    {
        ...
        $secret  = $this->secret();
        if ($secret !== '') {
            $headers['X-Pezhkam-Secret'] = $secret;
        }
    ...
    {
        return $this->send(['TEST', 'Pezhkam', '-', 'test', '-', current_time('mysql'), 'connection test', '-', 'test']);
    }
