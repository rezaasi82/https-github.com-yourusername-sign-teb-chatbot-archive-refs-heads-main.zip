 ...
 *
 * A lead is a conversation (pzk_conversations) plus its messages
 * (pzk_messages). This is the single representation consumed by every export
...
class LeadPayload
{
    ...
    public function __construct(?\Pezhkam\Database\ConversationRepository $conversations = null, ?\Pezhkam\Database\MessageRepository $messages = null)
    {
        $this->conversations = $conversations ?? new \Pezhkam\Database\ConversationRepository();
        $this->messages      = $messages ?? new \Pezhkam\Database\MessageRepository();
    }
