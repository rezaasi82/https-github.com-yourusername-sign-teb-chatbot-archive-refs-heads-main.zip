 ...
 * under uploads/pezhkam-pdf/YYYY/MM/ with an unguessable name.
 *
 * @package Pezhkam
 */
...
class PdfGenerator
{
    ...
    private \Pezhkam\Export\LeadPayload $payloads;

    public function __construct(?\Pezhkam\Core\Settings $settings = null, ?\Pezhkam\Export\LeadPayload $payloads = null)
    {
        $this->settings = $settings ?? new \Pezhkam\Core\Settings();
        $this->payloads = $payloads ?? new \Pezhkam\Export\LeadPayload();
    }
    ...
    private function target_dir(): ?array
    {
        ...
        $sub  = '/pezhkam-pdf/' . gmdate('Y') . '/' . gmdate('m');
        $path = $uploads['basedir'] . $sub;
        if (! wp_mkdir_p($path)) {
            return null;
        }
        // Prevent directory listing.
        if (! file_exists($uploads['basedir'] . '/pezhkam-pdf/index.php')) {
            file_put_contents($uploads['basedir'] . '/pezhkam-pdf/index.php', "<?php\n// Silence is golden.\n");
        }
    ...
    public function render_html(array $payload): string
    {
        ...
        foreach ($payload['messages'] as $m) {
            $is_user = $m['role'] === 'user';
            $who     = $is_user ? 'بیمار' : 'دستیار Pezhkam';
            $side    = $is_user ? 'pzk-p' : 'pzk-a';
...
<style>
    * { box-sizing: border-box; }
    ...
    .pzk-p { background: #eef2fb; border: 1px solid #dbe2f2; margin-inline-start: auto; }
    .pzk-a { background: #fff; border: 1px solid #e6e9f0; }
...
<div class="report">
    <div class="head">
        <div class="brand"><?php echo esc_html($clinic); ?><small>PEZHKAM</small></div>
    ...
    <div class="foot">
        <span>Pezhkam · <?php echo esc_html($exported); ?></span>
