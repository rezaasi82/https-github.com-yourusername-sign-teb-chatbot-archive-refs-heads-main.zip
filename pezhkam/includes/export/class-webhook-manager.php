...
{
    public const OPTION_SECRET = 'pzk_webhook_secret_enc';
    public const MAX_ATTEMPTS  = 3;

    private \Pezhkam\Core\Settings $settings;
    private \Pezhkam\Export\ExportLogger $logger;
    private \Pezhkam\Export\LeadPayload $payloads;

    public function __construct(?\Pezhkam\Core\Settings $settings = null, ?\Pezhkam\Export\ExportLogger $logger = null, ?\Pezhkam\Export\LeadPayload $payloads = null)
    {
        $this->settings = $settings ?? new \Pezhkam\Core\Settings();
        $this->logger   = $logger ?? new \Pezhkam\Export\ExportLogger();
    ...
    public static function save_secret(string $plain): void
    {
        ...
        update_option(self::OPTION_SECRET, \Pezhkam\Core\Encryption::encrypt($plain), false);
    }
    ...
    public function dispatch(int $lead_id, string $event, string $pdf_url = '', int $attempt = 1): array
    {
        ...
        if ($should_retry) {
            wp_schedule_single_event(time() + 300 * $attempt, 'pzk_webhook_retry', [$lead_id, $event, $pdf_url, $attempt + 1]);
        }
    ...
    public function send(array $payload, string $event = 'manual'): array
    {
        ...
        $headers = [
            ...
            'X-Pezhkam-Event'  => $event,
        ];
        if ($secret !== '') {
            $headers['X-Pezhkam-Signature'] = 'sha256=' . hash_hmac('sha256', (string) $body, $secret);
        }
