...
class MessengerNotifier
{
    ...
    private const CHANNELS = [
        'bale'     => ['https://tapi.bale.ai', 'pzk_msgr_bale_token_enc', 'بله'],
        'telegram' => ['https://api.telegram.org', 'pzk_msgr_tg_token_enc', 'Telegram'],
    ];
    ...
    {
        add_action('pzk_lead_detected', [$this, 'notify_new_lead'], 10, 2);
        add_action('wp_ajax_pzk_test_messenger', [$this, 'ajax_test']);
    }
    ...
    public function send(string $channel, string $text): array
    {
        ...
        if ($token === '' || $chat === '') {
            return ['ok' => false, 'error' => __('توکن ربات یا شناسه چت تنظیم نشده است.', 'pezhkam')];
        }
        ...
        return ['ok' => false, 'code' => $code, 'error' => $desc !== '' ? $desc : sprintf(__('پاسخ سرویس: HTTP %d', 'pezhkam'), $code)];
    }
    ...
    public function notify_new_lead(int $conversation_id, string $cta = ''): void
    {
        ...
        $c = (new \Pezhkam\Database\ConversationRepository())->get($conversation_id);
        if (! $c) {
            return;
        }
        ...
        foreach ($enabled as $channel) {
            ...
            \Pezhkam\Security\AuditLog::record('messenger_notify', ['object' => $channel, 'severity' => ! empty($r['ok']) ? 'info' : 'warning']);
        }
    }
    ...
    private function lead_text(object $c, string $cta): string
    {
        ...
        if ($cta !== '') {
            $lines[] = sprintf(__('اقدام: %s', 'pezhkam'), $cta);
        }
        if ($vars['summary'] !== '') {
            $lines[] = "\n" . $vars['summary'];
        }
    ...
    public function ajax_test(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \Pezhkam\Security\Security::note_failure('messenger_test');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        $channel = \Pezhkam\Core\Input::post_key('channel');
        if (! isset(self::CHANNELS[$channel])) {
