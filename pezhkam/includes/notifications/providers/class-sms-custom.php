...
class SmsCustom extends \Pezhkam\Notifications\SmsProviderBase
{
    ...
    {
        return __('سفارشی / خارجی (Custom HTTP)', 'pezhkam');
    }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($url === '') {
            return ['ok' => false, 'error' => __('آدرس سرویس سفارشی تنظیم نشده است.', 'pezhkam')];
        }
        ...
        return ['ok' => false, 'code' => $r['code'], 'error' => $r['error'] ?? sprintf(__('پاسخ سرویس: HTTP %d', 'pezhkam'), $r['code'])];
    }
