...
class SmsKavenegar extends \Pezhkam\Notifications\SmsProviderBase
{
    ...
    public function check(): array
    {
        $key = $this->api_key();
        if ($key === '') {
            return ['ok' => false, 'detail' => __('هیچ APIKey ذخیره نشده است.', 'pezhkam')];
        }
        $r = $this->http('https://api.kavenegar.com/v1/' . rawurlencode($key) . '/account/info.json', ['method' => 'GET']);
        if (! empty($r['error'])) {
            return ['ok' => false, 'detail' => sprintf(__('سرور سایت به پنل دسترسی ندارد: %s', 'pezhkam'), $r['error'])];
        }
    ...
    public function send(string $to, string $text): array
    {
        ...
        if ($key === '' || $to === '') {
            return ['ok' => false, 'error' => __('کلید API یا شماره مقصد تنظیم نشده است.', 'pezhkam')];
        }
    ...
    public function send_pattern(string $to, string $code, array $params): array
    {
        ...
        $to  = $this->normalize($to);
        if ($key === '' || $to === '' || $code === '') {
            return ['ok' => false, 'error' => __('کلید API، شماره یا کد الگو تنظیم نشده است.', 'pezhkam')];
        }
