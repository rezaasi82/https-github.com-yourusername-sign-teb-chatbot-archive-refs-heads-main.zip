...
class ChatController
{
    ...
    public function handle_message(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $result = (new \Pezhkam\Ai\AiManager())->handle([
            'session_id' => \Pezhkam\Rest\Sanitizer::session_id((string) $request->get_param('session_id')),
            'message'    => sanitize_textarea_field((string) $request->get_param('message')),
            'name'       => \Pezhkam\Rest\Sanitizer::name((string) $request->get_param('name')),
            'phone'      => \Pezhkam\Rest\Sanitizer::phone((string) $request->get_param('phone')),
