...
{
    private const REST_NAMESPACE = 'pezhkam/v1';
    ...
    {
        \Pezhkam\Core\JsonGuard::arm();
        $payload = (new \Pezhkam\Export\LeadPayload())->build((int) $request['id']);
        if ($payload === null) {
    ...
    {
        \Pezhkam\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pezhkam\Export\ExportManager())->export_pdf((int) $request->get_param('lead_id')));
    }

    public function export_webhook(\WP_REST_Request $request): \WP_REST_Response
    {
        \Pezhkam\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pezhkam\Export\ExportManager())->export_webhook((int) $request->get_param('lead_id'), 'manual'));
    }

    public function export_google_sheet(\WP_REST_Request $request): \WP_REST_Response
    {
        \Pezhkam\Core\JsonGuard::arm();
        return new \WP_REST_Response((new \Pezhkam\Export\ExportManager())->export_google_sheet((int) $request->get_param('lead_id')));
    }
}
