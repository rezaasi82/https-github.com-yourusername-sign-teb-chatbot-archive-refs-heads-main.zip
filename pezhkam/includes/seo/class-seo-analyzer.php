...
class SeoAnalyzer
{
    ...
    {
        return \Pezhkam\Core\Cache::remember('seo_q_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            global $wpdb;
            $messages = \Pezhkam\Database\Schema::messages_table();
    ...
    {
        return \Pezhkam\Core\Cache::remember('seo_kw_' . $days . '_' . $limit, 600, function () use ($days, $limit) {
            $counts = [];
    ...
    {
        return \Pezhkam\Core\Cache::remember('seo_topics_' . $days, 600, function () use ($days) {
            $services = array_map(
                static fn($s) => (string) $s['name'],
                (new \Pezhkam\Ai\SystemPromptBuilder(new \Pezhkam\Core\Settings()))->services()
            );
            if ($services === []) {
                return [];
            }
