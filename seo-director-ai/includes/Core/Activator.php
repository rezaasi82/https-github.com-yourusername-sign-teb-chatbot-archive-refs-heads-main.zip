<?php
/**
 * Activation: create schema via dbDelta, seed defaults, map capabilities.
 *
 * @package SEODirector
 */
...
final class Activator {
	...
	private static function activate_single(): void {
		self::create_tables();
		self::seed_options();
		Capabilities::add_caps();
		update_option( 'sda_db_version', SDA_DB_VERSION );

		if ( ! wp_next_scheduled( 'sda_daily_sync' ) ) {
			// 03:00 site time, converted to UTC timestamp.
			$next = strtotime( 'tomorrow 03:00:00' ) - ( (int) get_option( 'gmt_offset' ) * HOUR_IN_SECONDS );
			wp_schedule_event( $next, 'daily', 'sda_daily_sync' );
		}
	...
	/**
	 * Full schema per DB design doc. dbDelta-compatible statements.
	 */
	public static function create_tables(): void {
		...
		$tables[] = "CREATE TABLE {$p}connections (
		...
		$tables[] = "CREATE TABLE {$p}properties (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL DEFAULT 1,
		...
		$tables[] = "CREATE TABLE {$p}gsc_daily_totals (
		...
		$tables[] = "CREATE TABLE {$p}gsc_query_daily (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			property_id BIGINT UNSIGNED NOT NULL,
			date DATE NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}gsc_page_daily (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			property_id BIGINT UNSIGNED NOT NULL,
			date DATE NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}gsc_page_query_weekly (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			property_id BIGINT UNSIGNED NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}ga4_daily (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			property_id BIGINT UNSIGNED NOT NULL,
			date DATE NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}psi_audits (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			page_hash BINARY(16) NOT NULL,
			page_path VARCHAR(750) NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}insights (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}opportunities (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}roadmap_tasks (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
			roadmap_scope VARCHAR(12) NOT NULL,
			period_start DATE NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}health_scores (
			site_id BIGINT UNSIGNED NOT NULL,
			date DATE NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}job_state (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
		...
		$tables[] = "CREATE TABLE {$p}agency_sites (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			site_id BIGINT UNSIGNED NOT NULL,
	...
	private static function seed_options(): void {
		add_option(
			...
			array(
				...
				'ai_monthly_budget'  => 500000, // tokens.
				'ai_language'        => 'auto',
				'gsc_top_n_queries'  => 5000,
				'gsc_top_n_pages'    => 2000,
				'retention_months'   => 16,
