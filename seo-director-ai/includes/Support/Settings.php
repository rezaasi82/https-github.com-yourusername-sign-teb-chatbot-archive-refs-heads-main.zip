...
final class Settings {
	...
	private function sanitizers(): array {
		return [
			...
			'alert_bale_token'     => static fn( $v ) => sanitize_text_field( (string) $v ),
			'alert_bale_chat'      => static fn( $v ) => sanitize_text_field( (string) $v ),
			'auto_update'          => static fn( $v ) => (bool) $v,
			'gads_developer_token' => static fn( $v ) => sanitize_text_field( (string) $v ),
			'gads_customer_id'     => static fn( $v ) => preg_replace( '/[^0-9\-]/', '', (string) $v ),
