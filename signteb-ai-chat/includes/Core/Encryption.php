...
/**
 * AES-256-CBC symmetric encryption keyed off WordPress' own salts so the API
 * key is never stored in plaintext in wp_options. If OpenSSL is unavailable
 * (rare on Iranian shared hosts), it degrades to a reversible obfuscation so
 * the plugin keeps working — never throws.
 */
class Encryption
{
    ...
    private static function key(): string
    {
        ...
        if ($salt === '') {
            $salt = (string) get_option('stmc_chat_fallback_salt');
            if ($salt === '') {
                $salt = wp_generate_password(64, true, true);
                update_option('stmc_chat_fallback_salt', $salt, false);
            }
        }
