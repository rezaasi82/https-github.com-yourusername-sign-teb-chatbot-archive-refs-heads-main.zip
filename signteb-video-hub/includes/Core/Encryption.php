...
/**
 * AES-256-CBC symmetric encryption keyed off WordPress' own salts so API keys
 * are never stored in plaintext in wp_options. If OpenSSL is unavailable it
 * degrades to a reversible obfuscation so the plugin keeps working.
 */
class Encryption
{
    ...
    private static function key(): string
    {
        ...
        if ($salt === '') {
            $salt = (string) get_option('stvh_fallback_salt');
            if ($salt === '') {
                $salt = wp_generate_password(64, true, true);
                update_option('stvh_fallback_salt', $salt, false);
            }
        }
