...
(function () {
	...
	document.querySelectorAll('.swc-tile-num[data-count]').forEach(function (el) {
	...
	if (typeof window.SWC_DASH === 'undefined') { return; }
	var badge = document.getElementById('swc-int-badge');
	if (!badge) { return; }

	var body = new URLSearchParams();
	body.append('action', 'swc_integrity_check');
	body.append('nonce', window.SWC_DASH.nonce);

	fetch(window.SWC_DASH.ajaxUrl, {
		method: 'POST',
		...
		.then(function (res) {
			if (res && res.ok && res.integrity) {
				...
				var tile = badge.closest('.swc-tile-int');
				if (tile) {
					tile.className = tile.className.replace(/is-\w+/g, '');
					tile.classList.add('swc-tile-int', 'is-' + res.integrity.level.replace(/[^a-z]/g, ''));
				}
			}
		})
		.catch(function () {});
