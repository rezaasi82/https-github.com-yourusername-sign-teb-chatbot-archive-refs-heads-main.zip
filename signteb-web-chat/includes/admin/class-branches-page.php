...
class BranchesPage
{
    ...
    public function handle_save(): void
    {
        ...
        if ($data['name'] !== '') {
            $repo = new \SignTeb\WebChat\Database\BranchRepository();
            $id   = absint($in['branch_id'] ?? 0);
            if ($id > 0 && $repo->exists($id)) {
                $repo->update($id, $data);
                \SignTeb\WebChat\Security\AuditLog::record('branch_updated', ['object' => 'branch#' . $id]);
            } else {
                $new = $repo->create($data);
                \SignTeb\WebChat\Security\AuditLog::record('branch_created', ['object' => 'branch#' . $new]);
            }
        }
    ...
    public function handle_delete(): void
    {
        ...
        $id = \SignTeb\WebChat\Core\Input::post_int('branch_id');
        if ($id > 0) {
            (new \SignTeb\WebChat\Database\BranchRepository())->delete($id);
            \SignTeb\WebChat\Security\AuditLog::record('branch_deleted', ['object' => 'branch#' . $id, 'severity' => 'warning']);
        }
    ...
    public function render(): void
    {
        ...
        $edit_id  = \SignTeb\WebChat\Core\Input::get_int('edit');
