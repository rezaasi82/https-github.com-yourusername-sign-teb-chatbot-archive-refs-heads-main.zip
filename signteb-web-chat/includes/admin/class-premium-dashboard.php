...
class PremiumDashboard
{
    ...
    public function verify_integrity_gate(): array
    {
        ...
        try {
            $env_ok = defined('SWC_VERSION')
                && defined('SWC_FILE')
                && class_exists('\SignTeb\WebChat\Core\Plugin');
    ...
    private function metrics(int $days): array
    {
        ...
        $data = \SignTeb\WebChat\Core\Cache::remember('dash_metrics_' . $days, 300, function () use ($days) {
            $m = [
            ...
            try {
                ...
                $avg_price    = (int) (new \SignTeb\WebChat\Core\Settings())->get('avg_service_price', 0);
