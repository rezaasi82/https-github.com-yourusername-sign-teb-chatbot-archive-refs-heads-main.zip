...
class SettingsPage
{
    ...
    {
        $tab = \SignTeb\WebChat\Core\Input::get_key('tab', 'provider');
        return in_array($tab, self::TABS, true) ? $tab : 'provider';
    }

    public function handle_save(): void
    {
        if (! \SignTeb\WebChat\Core\Input::has_post('swc_settings_submit')) {
            return;
        }
        ...
        $in  = \SignTeb\WebChat\Core\Input::post_fields();
        ...
        if ($tab === 'provider') {
            ...
            if (isset($in['api_key_anthropic']) && trim((string) $in['api_key_anthropic']) !== '') {
                \SignTeb\WebChat\Core\Settings::save_api_key('anthropic', sanitize_text_field((string) $in['api_key_anthropic']));
            }
            if (isset($in['api_key_openai']) && trim((string) $in['api_key_openai']) !== '') {
                \SignTeb\WebChat\Core\Settings::save_api_key('openai', sanitize_text_field((string) $in['api_key_openai']));
            }
            if (isset($in['api_key_gapgpt']) && trim((string) $in['api_key_gapgpt']) !== '') {
                \SignTeb\WebChat\Core\Settings::save_api_key('gapgpt', sanitize_text_field((string) $in['api_key_gapgpt']));
            }
        } elseif ($tab === 'clinic') {
        ...
        update_option(\SignTeb\WebChat\Core\Settings::OPTION, array_merge($existing, $update));
        $this->finish($tab);
    }
    ...
    private function save_integrations(array $in): void
    {
        ...
        if (isset($in['sms_key']) && trim((string) $in['sms_key']) !== '') {
            \SignTeb\WebChat\Notifications\SmsManager::save_key((string) $in['sms_key']);
        }
        if (isset($in['sms_secret']) && trim((string) $in['sms_secret']) !== '') {
            \SignTeb\WebChat\Notifications\SmsManager::save_secret((string) $in['sms_secret']);
        }
        ...
        foreach (['bale', 'telegram'] as $ch) {
            if (isset($in['msgr_' . $ch . '_token']) && trim((string) $in['msgr_' . $ch . '_token']) !== '') {
                \SignTeb\WebChat\Notifications\MessengerNotifier::save_token($ch, (string) $in['msgr_' . $ch . '_token']);
            }
        }

        if (isset($in['webhook_secret']) && trim((string) $in['webhook_secret']) !== '') {
            \SignTeb\WebChat\Export\WebhookManager::save_secret((string) $in['webhook_secret']);
        }
        if (isset($in['gsheet_secret']) && trim((string) $in['gsheet_secret']) !== '') {
            \SignTeb\WebChat\Export\GoogleSheets::save_secret((string) $in['gsheet_secret']);
        }
        if (isset($in['cloud_secret']) && trim((string) $in['cloud_secret']) !== '') {
            \SignTeb\WebChat\Cloud\CloudClient::save_secret((string) $in['cloud_secret']);
        }
    }

    private function finish(string $tab): void
    {
        \SignTeb\WebChat\Security\AuditLog::record('settings_saved', ['object' => $tab, 'severity' => 'info']);
    ...
    public function render(): void
    {
        ...
        if ($float && $shortcode) {
            $display = __('ویجت شناور + شورت‌کد', 'signteb-web-chat');
        } elseif ($float) {
            $display = __('فقط ویجت شناور', 'signteb-web-chat');
        } elseif ($shortcode) {
            $display = __('فقط شورت‌کد', 'signteb-web-chat');
        } else {
            $display = __('نمایش خاموش', 'signteb-web-chat');
        }
        ...
        if (\SignTeb\WebChat\Core\Input::get_key('updated') === '1') {
        ...
        if (in_array($tab, ['provider', 'clinic', 'appearance', 'integrations'], true)) {
            ...
            (new \SignTeb\WebChat\Admin\StatsPage())->render_inner();
        }
