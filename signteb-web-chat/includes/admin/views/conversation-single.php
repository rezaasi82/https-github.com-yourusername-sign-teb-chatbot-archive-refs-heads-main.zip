...
<?php else : ?>
    ...
    <div class="swc-crm-panel" data-lead="<?php echo esc_attr($conversation->id); ?>" data-nonce="<?php echo esc_attr(\SignTeb\WebChat\Crm\LeadCrm::nonce()); ?>">
        ...
        <?php
        $ref_text  = \SignTeb\WebChat\Crm\LeadCrm::referral_text($conversation);
        $sms_mgr   = new \SignTeb\WebChat\Notifications\SmsManager();
        $sms_ready = $sms_mgr->is_configured();
        $messengers = \SignTeb\WebChat\Notifications\SmsManager::messenger_links((string) ($conversation->patient_phone ?? ''), $ref_text);
        ?>
