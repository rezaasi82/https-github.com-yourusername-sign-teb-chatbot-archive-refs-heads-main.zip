...
<form method="post" action="<?php echo esc_url(admin_url('admin.php?page=swc-chat&tab=' . $tab)); ?>">
    ...
    <?php if ($tab === 'provider') : ?>
        <table class="form-table" role="presentation">
            <tr>
                <th><?php esc_html_e('نمایش ویجت در سایت', 'signteb-web-chat'); ?></th>
                <td>
                    <label>
                        <input type="checkbox" name="float_enabled" value="1" <?php checked($s->is_float_enabled()); ?>>
                        <?php esc_html_e('ویجت شناور روی همهٔ صفحه‌های سایت نمایش داده شود', 'signteb-web-chat'); ?>
                    </label>
                    <p class="description"><?php esc_html_e('آیکون گفتگو در گوشهٔ همهٔ صفحه‌ها ظاهر می‌شود.', 'signteb-web-chat'); ?></p>
                    ...
                    <label style="margin-top:12px;display:inline-block">
                        ...
                        <?php esc_html_e('شورت‌کد فعال باشد', 'signteb-web-chat'); ?>
                    </label>
                    <p class="description">
                        <?php
                        printf(
                    ...
                    <p class="description">
                        <?php esc_html_e('اگر هر دو گزینه خاموش باشند، چت در سایت نمایش داده نمی‌شود و پاسخ‌گویی هوش مصنوعی هم متوقف می‌ماند.', 'signteb-web-chat'); ?>
                    </p>
                </td>
            </tr>
            <tr>
    ...
    <?php elseif ($tab === 'integrations') : ?>
        <?php
        $webhook = new \SignTeb\WebChat\Export\WebhookManager();
        $gsheet  = new \SignTeb\WebChat\Export\GoogleSheets();
        ?>
        ...
        <?php $sms = new \SignTeb\WebChat\Notifications\SmsManager(); $sms_active = $sms->active_id(); ?>
        ...
        <table class="form-table" role="presentation">
            ...
            <tr>
                ...
                <td>
                    <input type="password" name="sms_key" value="" class="regular-text" autocomplete="new-password" placeholder="<?php echo \SignTeb\WebChat\Notifications\SmsManager::key() !== '' ? '•••••••• (ذخیره شده)' : esc_attr__('APIKey دریافتی از پنل پیامکی', 'signteb-web-chat'); ?>">
            ...
            <tr class="swc-sms-secret-row">
                ...
                <td><input type="password" name="sms_secret" value="" class="regular-text" autocomplete="new-password" placeholder="<?php echo \SignTeb\WebChat\Notifications\SmsManager::secret() !== '' ? '••••••••' : ''; ?>"></td>
            </tr>
            <tr>
        ...
        <?php $msgr = new \SignTeb\WebChat\Notifications\MessengerNotifier(); ?>
        <table class="form-table" role="presentation">
            <?php foreach ($msgr->channels() as $ch => $cdef) : ?>
                <tr>
                    ...
                    <td>
                        ...
                        <br>
                        <input type="password" name="msgr_<?php echo esc_attr($ch); ?>_token" value="" class="regular-text" autocomplete="new-password" placeholder="<?php echo \SignTeb\WebChat\Notifications\MessengerNotifier::token($ch) !== '' ? '•••••••• (توکن ربات)' : esc_attr__('توکن ربات', 'signteb-web-chat'); ?>" style="margin:4px 0">
