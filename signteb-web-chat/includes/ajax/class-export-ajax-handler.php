...
class ExportAjaxHandler
{
    ...
    public function sms_diag(): void
    {
        ...
        $this->guard();
        $diag = (new \SignTeb\WebChat\Notifications\SmsManager())->diagnose();
    ...
    public function test_sms(): void
    {
        \SignTeb\WebChat\Core\JsonGuard::arm();
        $this->guard();
        $to = \SignTeb\WebChat\Core\Input::post_text('to');
        if ($to === '') {
            wp_send_json(['ok' => false, 'error' => __('شماره مقصد را وارد کنید.', 'signteb-web-chat')], 400);
        }

        $sms = new \SignTeb\WebChat\Notifications\SmsManager();
    ...
    public function send_sms(): void
    {
        ...
        $lead_id  = \SignTeb\WebChat\Core\Input::post_int('lead_id');
        $to       = \SignTeb\WebChat\Core\Input::post_text('to');
        $tpl_key  = \SignTeb\WebChat\Core\Input::post_key('template', 'referral');
        if ($lead_id <= 0 || $to === '') {
            wp_send_json(['ok' => false, 'error' => __('لید یا شماره مقصد نامعتبر است.', 'signteb-web-chat')], 400);
        }

        $c = (new \SignTeb\WebChat\Database\ConversationRepository())->get($lead_id);
        if (! $c) {
    ...
    public function export_pdf(): void
    {
        ...
        $this->guard();
        $lead_id = \SignTeb\WebChat\Core\Input::post_int('lead_id');
        wp_send_json((new \SignTeb\WebChat\Export\ExportManager())->export_pdf($lead_id));
    }
    ...
    public function export_webhook(): void
    {
        ...
        $this->guard();
        $lead_id = \SignTeb\WebChat\Core\Input::post_int('lead_id');
        wp_send_json((new \SignTeb\WebChat\Export\ExportManager())->export_webhook($lead_id, 'manual'));
    }
    ...
    public function export_gsheet(): void
    {
        ...
        $this->guard();
        $lead_id = \SignTeb\WebChat\Core\Input::post_int('lead_id');
        wp_send_json((new \SignTeb\WebChat\Export\ExportManager())->export_google_sheet($lead_id));
    }
    ...
    public function test_webhook(): void
    {
        \SignTeb\WebChat\Core\JsonGuard::arm();
        $this->guard();
        wp_send_json((new \SignTeb\WebChat\Export\WebhookManager())->test());
    }
    ...
    public function test_gsheet(): void
    {
        \SignTeb\WebChat\Core\JsonGuard::arm();
        $this->guard();
        wp_send_json((new \SignTeb\WebChat\Export\GoogleSheets())->test());
    }
    ...
    public function bulk(): void
    {
        ...
        $op  = \SignTeb\WebChat\Core\Input::post_key('op');
        $ids = array_map('absint', (array) (\SignTeb\WebChat\Core\Input::post_fields()['ids'] ?? []));
        ...
        $queue = new \SignTeb\WebChat\Jobs\JobQueue();
        foreach ($ids as $id) {
    ...
    public function download_pdf(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_admin_referer(self::NONCE, 'nonce')) {
            \SignTeb\WebChat\Security\Security::note_failure('pdf_download');
            wp_die(esc_html__('دسترسی غیرمجاز.', 'signteb-web-chat'), '', ['response' => 403]);
        }
        $lead_id = \SignTeb\WebChat\Core\Input::get_int('lead_id');

        $manager = new \SignTeb\WebChat\Export\ExportManager();
        $conv    = (new \SignTeb\WebChat\Database\ConversationRepository())->get($lead_id);
        ...
        $filename = 'signteb-lead-' . $lead_id . ($is_pdf ? '.pdf' : '.html');
        nocache_headers();
     ...
     * inside the plugin's swc-pdf folder.
     */
    private function url_to_path(string $url): string
    {
        ...
        if (strpos($url, $uploads['baseurl'] . '/swc-pdf/') !== 0) {
            return '';
        }
        $path = $uploads['basedir'] . substr($url, strlen($uploads['baseurl']));
        $real = realpath($path);
        $base = realpath($uploads['basedir'] . '/swc-pdf');
    ...
    private function delete_files(int $lead_id): bool
    {
        ...
        (new \SignTeb\WebChat\Database\ConversationRepository())->set_pdf_url($lead_id, '');
        return true;
    }
}
