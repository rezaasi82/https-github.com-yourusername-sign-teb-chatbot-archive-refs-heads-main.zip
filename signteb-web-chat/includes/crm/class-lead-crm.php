...
class LeadCrm
{
    ...
    public function ajax_refer(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \SignTeb\WebChat\Security\Security::note_failure('crm_refer');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }

        $lead_id = \SignTeb\WebChat\Core\Input::post_int('lead_id');
        $to      = \SignTeb\WebChat\Core\Input::post_email('to');
        if ($lead_id <= 0) {
        ...
        \SignTeb\WebChat\Security\AuditLog::record('lead_refer', ['object' => (string) $lead_id, 'severity' => $sent ? 'info' : 'warning']);
    ...
    public function ajax_update(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \SignTeb\WebChat\Security\Security::note_failure('crm_update');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        ...
        if (\SignTeb\WebChat\Core\Input::has_post('lead_status')) {
            $status = \SignTeb\WebChat\Core\Input::post_key('lead_status');
            if (self::is_valid($status)) {
                $fields['lead_status'] = $status;
            }
        }
        if (\SignTeb\WebChat\Core\Input::has_post('email')) {
            $fields['email'] = \SignTeb\WebChat\Core\Input::post_email('email');
        }
        if (\SignTeb\WebChat\Core\Input::has_post('tags')) {
            $fields['tags'] = \SignTeb\WebChat\Core\Input::post_text('tags');
        }
        if (\SignTeb\WebChat\Core\Input::has_post('notes')) {
            $fields['notes'] = \SignTeb\WebChat\Core\Input::post_textarea('notes');
        }
        if (\SignTeb\WebChat\Core\Input::has_post('branch_id')) {
            $branch = \SignTeb\WebChat\Core\Input::post_int('branch_id');
            if ($branch === 0 || (new \SignTeb\WebChat\Database\BranchRepository())->exists($branch)) {
