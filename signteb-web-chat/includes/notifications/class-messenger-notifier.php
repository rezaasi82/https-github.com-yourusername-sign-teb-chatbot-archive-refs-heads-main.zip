...
class MessengerNotifier
{
    ...
    public function notify_new_lead(int $conversation_id, string $cta = ''): void
    {
        ...
        $c = (new \SignTeb\WebChat\Database\ConversationRepository())->get($conversation_id);
        if (! $c) {
            return;
        }
        ...
        foreach ($enabled as $channel) {
            ...
            \SignTeb\WebChat\Security\AuditLog::record('messenger_notify', ['object' => $channel, 'severity' => ! empty($r['ok']) ? 'info' : 'warning']);
        }
    }
    ...
    public function ajax_test(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \SignTeb\WebChat\Security\Security::note_failure('messenger_test');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        $channel = \SignTeb\WebChat\Core\Input::post_key('channel');
        if (! isset(self::CHANNELS[$channel])) {
