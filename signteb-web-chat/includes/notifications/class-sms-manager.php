...
namespace SignTeb\WebChat\Notifications;
...
class SmsManager
{
    ...
    private const PROVIDERS = [
        'kavenegar'   => '\SignTeb\WebChat\Notifications\Providers\SmsKavenegar',
        'melipayamak' => '\SignTeb\WebChat\Notifications\Providers\SmsMelipayamak',
        'smsir'       => '\SignTeb\WebChat\Notifications\Providers\SmsSmsir',
        'ghasedak'    => '\SignTeb\WebChat\Notifications\Providers\SmsGhasedak',
        'custom'      => '\SignTeb\WebChat\Notifications\Providers\SmsCustom',
    ];

    private \SignTeb\WebChat\Core\Settings $settings;

    public function __construct(?\SignTeb\WebChat\Core\Settings $settings = null)
    {
    ...
    public function active(): ?\SignTeb\WebChat\Notifications\SmsProviderInterface
    {
    ...
    public function send(string $to, string $text): array
    {
        ...
        return $this->dispatch(static fn(\SignTeb\WebChat\Notifications\SmsProviderInterface $p) => $p->send($to, $text));
    }
    ...
    public function send_lead(string $to, string $template_key, array $vars): array
    {
        ...
        return $this->dispatch(function (\SignTeb\WebChat\Notifications\SmsProviderInterface $p) use ($to, $tpl_text, $code, $vars, $order) {
    ...
    private function dispatch(callable $call): array
    {
        ...
        \SignTeb\WebChat\Security\AuditLog::record('sms_send', ['object' => $provider->id(), 'severity' => ! empty($result['ok']) ? 'info' : 'warning']);
        return $result;
    }
    ...
    public function vars_for_lead(object $c): array
    {
        ...
        return [
            ...
            'status'  => \SignTeb\WebChat\Crm\LeadCrm::label((string) ($c->lead_status ?? 'new')),
