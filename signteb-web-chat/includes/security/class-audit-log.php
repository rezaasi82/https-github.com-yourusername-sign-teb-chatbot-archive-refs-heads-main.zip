...
class AuditLog
{
    ...
    public static function record(string $action, array $args = []): void
    {
        ...
        $wpdb->insert(
            \SignTeb\WebChat\Database\Schema::audit_logs_table(),
            [
                'user_id'    => get_current_user_id() ?: null,
                'action'     => substr($action, 0, 48),
                'object'     => isset($args['object']) ? substr((string) $args['object'], 0, 64) : null,
                'severity'   => $severity,
                'ip'         => class_exists('\SignTeb\WebChat\Security\Security') ? \SignTeb\WebChat\Security\Security::ip() : null,
        ...
        if (wp_rand(1, 50) === 1) {
            $table = \SignTeb\WebChat\Database\Schema::audit_logs_table();
