...
class SeoPage
{
    ...
    public function render(): void
    {
        ...
        $analyzer  = new \SignTeb\WebChat\Seo\SeoAnalyzer();
    ...
    public function ajax_generate(): void
    {
        ...
        if (! current_user_can('manage_options') || ! check_ajax_referer(self::NONCE, 'nonce', false)) {
            \SignTeb\WebChat\Security\Security::note_failure('seo_generate');
            wp_send_json(['ok' => false, 'error' => 'unauthorized'], 403);
        }
        // Protect the API budget: a few generations per hour is plenty.
        if (! \SignTeb\WebChat\Security\Security::rate_limit('seo_generate', 10, HOUR_IN_SECONDS)) {
        ...
        \SignTeb\WebChat\Security\AuditLog::record('seo_generate', ['object' => 'ideas']);
    ...
    private function generate(): array
    {
        ...
        $provider = (new \SignTeb\WebChat\Ai\ProviderFactory($settings))->create_active();
        if ($provider === null) {
